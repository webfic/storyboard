# 변경 내역

Storyboard의 주요 변경 사항을 한국어로 기록합니다.

이 문서는 [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) 형식을 따르며,
첫 공개 릴리스 이후에는 [Semantic Versioning](https://semver.org/spec/v2.0.0.html)을 따릅니다.

영어 변경 내역은 [CHANGELOG.en.md](CHANGELOG.en.md)를 참고하세요.

## [Unreleased]

## [0.9.7] - 2026-09-22

### 추가

- **`apikey set` 이 키를 가려진 입력으로 받고 바로 확인합니다.** 지금까지는 stdin 을 그대로 읽어 타이핑한 키가 스크롤백에 남았고, 잘못 붙여 넣어도 첫 실제 호출에서야 알 수 있었습니다. 이제 터미널에서는 제공자를 번호로 고르고 키를 가려진 입력으로 받은 뒤, 저장 직후 연결을 확인해 성공·실패를 바로 알려 줍니다. 파이프로 넘길 때는 종전처럼 stdin 을 읽습니다. `apikey show` 는 어느 제공자에 키가 있는지 «있음/없음» 으로만 보여 주고 값은 어디에도 찍지 않습니다. (`setup` 은 기본 제공자까지 바꾸므로, 키만 넣으려면 이쪽이 맞습니다.)
- **Claude 호출에 접두 캐시가 걸립니다.** 긴 원고를 여러 기준으로 나눠 묻는 호출은 같은 원고를 매번 다시 보냈고, 캐시는 시스템 블록에만 걸려 있어 실제로는 한 번도 읽히지 않았습니다. 이제 되풀이되는 앞부분에 캐시 경계를 달아 재사용하며, 요금 계산도 캐시 쓰기 1.25배·읽기 0.1배로 캐시 토큰을 함께 셉니다 — 이전에는 캐시 토큰이 요금에서 빠져 있었습니다. OpenAI·ollama 에는 표시를 보내지 않습니다.
- **남겨 둔 초안을 다른 심판으로 다시 채점할 수 있습니다.** 심판을 바꿔 보고 싶을 때 생성을 다시 돌리지 않아도 되며, 심판 사용량은 별도 원장에 남습니다.

### 변경

- **측정 리포트가 직전 엔진 대비 변화량을 함께 보여 줍니다.** 지점마다 몰입도와 비평가 점수가 어느 방향으로 얼마나 움직였는지 한 줄에 나오므로, 고친 것이 실제로 나아졌는지 표만 보고 알 수 있습니다.
- **독자가 중간에 그만두지 않고 끝까지 읽으며, 몰입도 평균이 함께 나옵니다.**
- **비평가 관문을 실효화하고 기준을 중립으로 고쳤습니다.** 관문이 사실상 아무것도 걸러 내지 못하던 것과, 채점 기준이 «원고가 망가지는 방식» 을 특정 형태로 전제하던 것을 고쳤습니다. 관문 여유는 2점으로 낮췄습니다.

## [0.9.6] - 2026-09-16

### 고침

- **로컬 모델이 «생각»만 하다 끝났을 때 빈 원고가 저장되던 문제를 고쳤습니다.** `gemma4:12b` 가 뼈대 프롬프트에 생각 19,000자를 써서 출력 한도를 다 쓰고 본문은 빈 채로 돌아왔는데, 빈 답이 정상으로 처리돼 «내용이 입력되지 않았습니다» 한 줄이 원고로 저장됐습니다. 8화 중 2화가 그렇게 121자로 끝났는데 오류는 없었습니다. 이제 본문이 비고 생각만 있으면 생성 실패로 멈추고, 생각 길이와 대처(생각 끄기·출력 상한 올리기)를 메시지에 적으며 가짜 원고를 남기지 않습니다.
- **심판의 답이 잘리거나 곡선 따옴표로 끝나 읽히지 않던 경우를 고쳤습니다.**

### 추가

- **프롬프트 변형과 로컬 «생각» 스위치를 고를 수 있습니다.** `promptVariant`(`generic`·`xs`·`rich`)가 모델 이름 규칙보다 우선합니다 — 이전에는 로컬 qwen·gemma 계열이 이름만으로 가장 작은 프롬프트를 받아, 지금까지의 실측이 전부 그 프롬프트로 잰 값이었습니다. `providers.ollama.think` 로 생각을 켜고 끌 수 있습니다(생각하는 모델은 한 호출이 수십 분이 될 수 있습니다).
- **`sim apply` 가 실제로 모델 프로필을 씁니다.** 실측값을 옮기는 마지막 걸음이 비어 있어 결국 사람이 손으로 옮겼고, 그러면 어느 실행에서 온 값인지 반년 뒤에 알 수 없습니다. 이제 손잡이를 엔진의 프로필 파일에 «제공자:모델» 키로 적고 날짜·트랙·유효 회차·도달률·AUC·회수·심판을 함께 남깁니다. 이미 있는 프로필은 `--force` 없이 덮어쓰지 않습니다.
- **실측한 `gemma4:12b` 프로필을 넣었습니다.** 로컬 기본 모델이 바뀌었는데 파이프라인은 일반 기본값(구간 상한 7,000)으로 돌아, 이 모델이 목표의 3분의 1 길이로 쓰는 성향이 보정되지 않고 있었습니다. 구간 상한을 1,000 으로 적었습니다(스릴러 연쇄 트랙, 유효 3회, 도달률 0.70).
- **측정 지점에 이름을 붙이고 생성기별로 이어 돌립니다.** 같은 지점의 다른 실험이 기록에서 한 줄로 합쳐지던 것과, 이어 돌리기가 생성기 차이를 보지 않던 것을 함께 고쳤습니다.

## [0.9.5] - 2026-09-16

### 고침

- **Ollama 에 설정해 둔 모델이 조용히 기본 모델로 바뀌던 문제를 고쳤습니다.** 0.9.4 가 카탈로그에서 `qwen3:14b`·`gemma3:12b` 를 빼면서, 카탈로그에 없는 모델을 설정해 두면 과제마다 기본 모델(`gemma4:12b`)을 부르게 됐습니다. 그 모델이 기계에 없으면 호출마다 404 로 죽고, 있었다면 고르지 않은 모델로 조용히 생성됐습니다. 0.9.4 가 «목록에 없는 태그도 그대로 씁니다» 라고 적은 동작이 실제로는 지켜지지 않던 것입니다.
- **심판의 답을 읽지 못해 회차가 사라지던 경우들을 고쳤습니다.** 근거 인용을 따옴표·홑화살괄호까지 견주어 본문에 있는 인용을 없는 것으로 보거나, 인용 안의 따옴표가 JSON 을 깨뜨리거나, 여러 문장을 이어 만든 인용이 본문에 다 있는데도 걸리던 경우입니다. 되묻기도 어느 인용이 왜 거부됐는지 알려 주고 그 인용만 좁게 다시 받도록 바꿨습니다.
- **하한선 관문의 판정을 믿을 수 있게 고쳤습니다.** 후보를 모든 독자에게 같은 순서로 보여 주어 관문이 제 라벨을 채점하던 것, 순서를 바꿔 물었을 때 뒤집히는 답을 그대로 세던 것, 순위만 매기고 근거를 묻지 않던 것을 고쳤습니다.

### 변경

- **하한선 관문이 회차를 버리지 않고 표시로 남깁니다.** 작은 심판이 보통 원고와 훼손본을 일관되게 가르지 못해 회차 대부분이 관문에서 사라졌고, 45분짜리 8화 판정이 관문 한 번에 통째로 없어졌습니다. 이제 관문 결과를 기록에 남기고 곡선은 계속 읽으므로 «관문 통과분» 과 «전체» 를 나란히 볼 수 있습니다. 답 자체를 읽을 수 없을 때만 폐기합니다.

### 추가

- **로컬 런타임에 모델이 없으면 시작 전에 거부합니다.** ollama 에 없는 모델을 부르면 호출마다 404 가 나는데 실행기는 씬마다 실패를 적고 넘어가, 자리를 비운 몇 시간이 빈 기록으로 사라졌습니다. 이제 첫 호출 전에 설치된 모델을 확인하고 무엇이 없고 무엇이 있는지 적어 멈춥니다.

## [0.9.4] - 2026-09-14

### 추가

- **서술자 카드를 폼으로 편집합니다.** `narrator/*.card` 를 카드 에디터로 열면 배경 카드 폼(Aliases·Weather·수집 탭)이 뜨고 인칭·지식 범위·시제를 폼에서 고칠 수 없었습니다. 이제 서술자 전용 입력과 미리보기가 나옵니다.
- **회차마다 씬 초안을 결과 옆에 남깁니다.** 측정은 사본에서 돌고 사본은 회차가 끝나면 지워져, 하한선 관문이 생성본을 훼손본 아래로 밀었을 때 심판이 옳았는지 원고가 정말 나빴는지 가릴 근거가 없었습니다.

### 변경 (호환성 주의)

- **모델 목록과 요금을 2026-09-13 기준으로 갱신했습니다.** 새로 고를 수 있는 모델: GPT-6 Astra·GPT-5.6 Sol/Terra/Luna·GPT-5.5, Claude Opus 5·Fable 5.1·Opus 4.8, Gemini 3.8 Flash·3.1 Pro(Preview)·3.5 Flash-Lite, Grok 4.20(추론·비추론), Ollama `gemma4`·`qwen3.5`·`exaone3.5`·`gpt-oss`. 틀렸던 요금(GPT-5.4 mini, Gemini 2.5 Flash·Flash-Lite)도 공식 요금으로 고쳤습니다.
- **기본 모델이 바뀝니다.** 모델을 직접 고른 적이 없다면 OpenAI는 `gpt-5.6-terra`, Gemini는 `gemini-3.8-flash`, Ollama는 `gemma4:12b`로 생성합니다(Claude·Grok은 그대로).
- **종료됐거나 종료 예정인 모델을 뺐습니다.** `gpt-5-mini`·`gpt-5-nano`(2026-12-11 종료), `claude-sonnet-4-5`(2026-09-29 종료 가능), 그리고 옛 Ollama 제안 태그(`qwen3`·`gemma3`·`llama3.x`·`qwen2.5`)입니다. 이 모델을 설정해 두었다면 제공자 기본 모델로 생성되니 설정에서 다시 골라 주세요. Ollama는 목록에 없는 태그도 그대로 씁니다.
- **Mock AI 가 설정 화면의 제공자 목록에서 빠집니다.** 개발용 제공자가 기본 제공자·태스크 제공자 드롭다운, 연결 탭, 제공자 선택 명령에 작가용 선택지와 나란히 있었습니다. 이미 mock 으로 저장해 둔 값은 그대로 보입니다.

### 고침

- **`temperature`를 거부하는 모델로 생성이 실패할 수 있던 문제를 고쳤습니다.** Claude 4.7 이후 모델(Sonnet 5 포함)·Gemini 3.x·GPT-6 Astra에는 `temperature`를 보내지 않습니다.
- **측정 결과가 실제 점수 대신 0 으로 보이던 문제를 고쳤습니다.** 유효 회차가 점수를 냈는데도 `sim report` 가 «AUC 0.000» 을 보였습니다 — 폐기된 회차를 0 점으로 함께 평균 냈기 때문입니다. 엔진·트랙이 다른 기록이 한 지점으로 합쳐지던 것과, `sim apply` 가 폐기 회차까지 세던 것도 함께 고쳤습니다.
- **심판이 지정한 런타임으로 가지 않던 문제를 고쳤습니다.** `sim.config.json` 의 ollama 주소와 문맥 창이 생성에만 닿고 심판에는 닿지 않아, 주소가 같은 기계에서는 문맥 창 없이 판정했습니다. 8화 누적 프롬프트가 모델 기본 문맥을 넘으면 앞부분이 오류 없이 잘립니다.
- **인용 판정이 따옴표 모양에 걸리던 문제를 고쳤습니다.** 근거 인용을 글자가 아니라 따옴표·홑화살괄호까지 견주어, 본문에 있는 인용이 없는 것으로 판정되곤 했습니다. 장르 독자 한 명만 근거를 잃어도 회차 전체를 버리던 것과, 하한선 후보가 모든 독자에게 같은 순서로 나가 라벨을 채점하던 것도 함께 고쳤습니다.
- **엔진을 패키지 버전이 아니라 git 커밋으로 기록합니다.** 하루에 빌드를 다섯 번 해도 기록이 전부 «0.9.3» 으로 남아, 어느 수정 뒤의 값인지 알 수 없었습니다.

## [0.9.3] - 2026-09-13

### 변경 (호환성 주의)

- **저장된 옛 제공자 이름을 더 이상 자동으로 옮기지 않습니다.** 0.9.2 는 `claude-code`·`codex`·`gemini-cli` 로 저장된 값을 읽는 시점에 `claude`·`openai`·`google` 로 옮겨 주었는데, 그러면 사용자가 고른 적 없는 모델과 요금으로 청구될 수 있습니다. 이제 옛 이름은 «고르지 않음» 으로 읽혀 생성이 거부되고, 어느 제공자를 쓸지 다시 고르시게 됩니다. `storyboard setup` 이나 설정 화면에서 한 번 고르면 됩니다.

### 추가

- **측정 명령이 생겼습니다.** `storyboard sim run|screen|sweep|report|apply` 로 생성 손잡이(프롬프트 온도·출력 상한·뼈대 비율·구간 상한 등 98개)를 실제 원고로 재고, L9 격자로 훑고, 파레토 경계를 그려 좋은 조합을 고른 뒤 되쓸 수 있습니다. 돈이 드는 동사는 견적을 찍고 멈추며 `--yes` 가 있어야 한 호출이라도 합니다. 채점은 생성과 **다른** 제공자로만 돌아갑니다 — 자기 글을 자기가 채점하면 그 지점만 점수가 들뜨기 때문입니다. 되쓰기를 별도 동사로 둔 것도 같은 이유로, 몇 시간짜리 실행이 사람 없는 사이에 소스를 바꾸지 않게 하기 위해서입니다.
- **로컬 런타임(ollama)으로 전체를 돌릴 수 있습니다.** `providers.ollama.contextTokens` 로 문맥 창을 지정하면 호출에 실립니다 — 이전에는 모델 기본 문맥을 넘는 프롬프트의 앞부분이 오류 없이 잘렸고, 8화를 누적해 읽는 채점에서 특히 위험했습니다. 기계마다 VRAM 이 다르므로 기본값은 두지 않았습니다. 로컬은 받아 둔 모델이 기계마다 다르므로 모델 목록이 «고정»에서 «제안»으로 바뀌어 카탈로그에 없는 이름도 받습니다(종량제 제공자는 종전대로 검사합니다). 기본 모델은 `qwen3:8b` 입니다.

### 고침

- **도움말이 이미 사라진 제공자를 예시로 들던 문제를 고쳤습니다.** `--provider` 설명과 `setup`·`config set` 예시가 `codex`·`claude-code` 를 고를 수 있는 것처럼 보여, 그대로 따라 치면 거부됐습니다.
- **`doctor` 의 분량 미달 경고가 사라진 모델을 조치로 안내하던 문제를 고쳤습니다.** 같은 문단이 이미 권하던 구간 상한 낮추기로 바꿨습니다.

## [0.9.2] - 2026-09-12

### 제거

- **구독형 CLI 제공자(Claude Code·Codex·Gemini CLI)를 뺐습니다.** Anthropic·OpenAI·Google 모두 구독과 계정 로그인을 대화형 개인 사용으로 한정하고, 프로그램적·대량 호출에는 API 키를 쓰도록 안내합니다. Storyboard의 장편 생성과 CLI 앱의 무인 실행은 후자에 해당하므로, 경계를 코드로 지키는 대신 경로를 API 키 하나로 정리했습니다. 함께 사라진 것: CLI 실행 명령·타임아웃·추론 강도 설정, 설정 화면의 «실행 명령» 칸과 «CLI 미설치» 상태, 그리고 사용 한도 폴백(`--fallback`).
- **`--fallback` 옵션을 뺐습니다.** 구독 CLI가 기간 한도에 걸렸을 때만 쓰이던 장치라 남길 이유가 없어졌습니다.

### 변경

- **기존 설정은 그대로 두어도 됩니다.** `claude-code`·`codex`·`gemini-cli`로 저장된 값은 읽는 시점에 각각 `claude`·`openai`·`google`로 옮겨집니다. 다만 명령줄에 직접 적은 `--provider codex`는 조용히 바꾸지 않고 대체 제공자를 알려 주며 거부합니다.
- **Claude 기본 모델이 Sonnet 5(`claude-sonnet-5`)가 됐습니다.** 100만 토큰당 입력 $2 / 출력 $10입니다.

## [0.9.1] - 2026-09-11

### 고침

- **시점 교차 구성이 작품 시점과 어긋난 서술자 카드를 만들던 문제를 고쳤습니다.** 프리셋이 시점에서 인칭·지식 경계를 따로 계산해, 2인칭 작품에는 3인칭 서술자 카드가, 전지적 시점 작품에는 목격 범위 서술자 카드가 만들어졌습니다. 이제 형식이 정의한 파생 규칙 하나만 씁니다.
- **`storyboard config set` 이 `timeoutMs`·`reasoningEffort` 를 거부하던 문제를 고쳤습니다.** 받을 수 있는 칸의 목록과 실제 검사가 따로 적혀 있어 두 칸이 검사에서 빠져 있었습니다.
- **설정 화면에 보이는 기본값·허용 범위와 실제 생성에 쓰이는 값이 갈라질 수 있던 문제를 고쳤습니다.** 설정 하나의 기본값과 범위가 세 곳에 따로 적혀 있어, 한쪽만 고치면 조용히 어긋났습니다.

### 변경

- **제공자 이름 표기가 한 곳에서 나옵니다.** 여덟 제공자가 «키 없음·모델 없음·연결 실패·생성 실패» 문장을 각자 적고 있어서, xAI·Claude Code 등 일부 제공자의 이름이 화면마다 달랐습니다. 이제 모두 카탈로그의 표시명을 씁니다.

### 내부

- 흩어져 있던 값에 소유자를 하나씩 정했습니다. 제공자 표(모델·요금·기본 명령), 비평 카테고리, 경로·확장자·감시 글롭, CLI 도움말과 자동완성 목록, 확장 기여 id, 반복되는 안내 문구가 각각 한 표에서 나옵니다. 프롬프트 35개의 temperature·토큰 예산과 생성 파이프라인 기본값은 `*.params.json` 파일로 모아 실측으로 조정할 자리를 한곳에 두었습니다.
- 소유자가 정해진 문자열이 다른 파일에 다시 나타나면 아키텍처 검사가 빌드를 실패시킵니다. 규칙을 적어 두는 것만으로는 다음 사람이 리터럴을 다시 적는 것을 막지 못하기 때문입니다.

## [0.9.0] - 2026-09-10

### 제거

- **Telegram 봇(`storyboard-bot`)이 사라졌습니다.** 텔레그램에서 워크스페이스를 조회·편집·생성하던 앱을 저장소에서 걷어냈습니다. 봇만을 위해 존재하던 «저장 = 커밋» 쓰기 모델(mutate gate·신선도 가드)과 커밋·동기화 패키지(`@storyboard/story-git`), 잡 큐와 네이티브 의존(`better-sqlite3`)도 함께 사라집니다. 남는 것은 VSCode 확장과 CLI 둘이고, 릴리즈 산출물도 VSIX 와 CLI tarball 둘입니다. 폰에서 초안을 읽는 방법은 워크스페이스를 git 클라이언트로 클론하는 것이며, 이제 그 저장소는 `storyboard init` 이 만들어 줍니다.

### 추가

- **`storyboard init` 이 git 저장소도 만듭니다.** 지금까지 워크스페이스를 저장소로 만들어 주는 것은 봇의 `/doctor init` 뿐이어서, 확장이나 CLI 로 시작한 작품은 사용자가 직접 `git init` 을 해야 했습니다. 이제 `init` 과 `init --repair` 가 `.gitignore` 를 먼저 쓴 뒤 저장소가 아니면 `git init` 을 실행하므로, 생성물이 첫 커밋에 쓸려 들어갈 여지 없이 작품이 처음부터 이력 위에서 시작합니다. 첫 커밋은 여전히 사용자의 몫이고, `storyboard doctor` 가 저장소가 아닌 워크스페이스를 짚어 줍니다.

## [0.8.10] - 2026-09-10

### 추가

- **이야기 상태 원장을 다시 봉인할 수 있습니다.** 카드를 고쳤지만 지금 초안이 맞다고 판단했을 때, `storyboard state reseal [<씬 범위>]` 와 익스텐션의 **Storyboard: Reseal Story State** 가 그 판단을 원장에 남깁니다. 지금까지는 낡음 표시를 지우는 길이 초안 재생성뿐이어서, 비트만 채워 넣어도 그 씬이 남긴 사실이 통째로 프롬프트에서 빠졌습니다. 봉인은 되돌릴 수 없으므로 익스텐션에서는 대상 씬을 보여 주고 확인을 받습니다.
- **`doctor` 가 모델과 씬 목표 분량이 맞는지 봅니다.** 씬 목표가 큰 작품에서 실측상 목표를 못 채우는 모델을 쓰고 있으면 그 사실과 실측 달성률을 알려 줍니다. 생성에 수십 분을 쓴 뒤에야 분량 미달을 발견하는 일을 줄입니다. 재보지 않은 프로바이더·모델 조합에 대해서는 아무 말도 하지 않습니다.

### 변경

- **모델마다 실측으로 정한 생성 파라미터를 씁니다.** 씬을 몇 구간으로 나눠 쓸지 같은 값이 모델마다 다르게 최적입니다. Opus 는 목표 15,000자 씬을 **한 번에** 쓸 때 가장 좋아(목표의 101%, 호출 1회) 구간 상한이 15,000으로 잡힙니다 — 종전 3회 호출로 92%였던 것보다 분량은 늘고 비용은 1/3입니다. 뼈대에 배분하는 비율도 모델마다 반대로 최적입니다: Sonnet 은 0.8로 올려야 목표의 44%→61%로 오르고, Opus 는 같은 값에서 137%까지 넘쳐 종전 1/3이 맞습니다. 재보지 않은 모델은 종전 기본값 그대로이고, `config show` 가 그 값의 출처를 «모델 실측»으로 구분해 보여 줍니다. 사용자가 지정한 값은 언제나 우선합니다.
- **`claude-code` 의 기본 모델이 Opus 로 바뀌었습니다.** 실측에서 씬 목표 15,000자 작품의 초안이 Sonnet 은 목표의 47%, Opus 는 92%였습니다. 기본값이 목표를 못 맞추면 원인을 파이프라인에서 찾게 되므로 기본을 Opus 로 두었습니다. 비용을 아끼려면 `storyboard config set providers.claude-code.model sonnet` 또는 설정 패널에서 되돌릴 수 있습니다.
- **설정을 git 처럼 저장합니다.** `storyboard config set` 과 `storyboard setup` 은 워크스페이스 안에서 실행하면 그 작품의 `.storyboard/config.json` 에 쓰고, 모든 작품에 걸려면 `--global` 을 붙입니다. 워크스페이스 밖에서 플래그 없이 부르면 어디에 쓸지 모호하므로 거부합니다. 예전에는 언제나 공용 파일에 써서 작품별 설정을 두려면 파일을 손으로 만들어야 했습니다. API 키는 종전대로 0600 홈 파일 하나에만 저장됩니다.

### 고침

- **비트를 채운 씬의 초안이 오히려 짧아지던 문제.** 씬 카드에 비트가 있으면 뼈대 단계가 카드의 목적·갈등·반전·감정 변화 블록을 통째로 받지 못했습니다. 사건 재료만 남기려던 필터가 설계 블록까지 함께 버렸기 때문입니다. 이제 설계 블록은 «장면 설계»로 따로 전달되어 사건 재료와 섞이지 않으면서도 뼈대에 도달합니다. 실측(sonnet, 목표 3,000자)에서 뼈대가 743자 → 1,900자, 초안이 1,636자 → 2,928자(목표의 98%)로 늘고, 비트가 있을 때와 없을 때의 분량 차이가 37%에서 0.4%로 사라졌습니다. 사건 커버리지는 그대로였습니다.
- **`doctor` 가 존재하지 않는 명령을 안내하던 문제.** 이야기 상태가 낡았을 때 `storyboard draft generate` 를 실행하라고 알려 줬지만 그런 명령이 없어 그대로 치면 실패했습니다. 실제 해소 명령인 `storyboard scene generate --all` 로 고쳤습니다.

## [0.8.9] - 2026-09-07

### 추가

- **`storyboard-bot` 에 터미널 명령이 생겼습니다.** `storyboard-bot doctor` 가 설정 파일·워크스페이스·git·프로바이더를 점검하고, `--help`·`--version` 이 사용법과 버전을 알려 줍니다. 텔레그램의 `/doctor` 와 달리 봇이 뜨지 않는 상태에서도 답하므로, 설정이 없거나 깨졌을 때 원인을 터미널에서 바로 볼 수 있습니다.
- **캐넌의 유효 구간을 이야기 시간으로 잽니다.** 씬 카드에 정수 `storyTime`을 적으면 유효 구간을 그 축으로 재므로, 회상·액자·시간 점프가 있어도 «20화에서 죽은 인물이 25화 회상에서도 죽어 있는» 일이 없습니다. 적지 않은 씬은 종전대로 씬 순번을 쓰고, 시간축을 쓰는 사실이 하나도 없으면 씬을 읽지도 않습니다. 공개 시점(`revealFrom`)은 독자가 읽은 순서의 문제라 씬 순번으로 남겼고, `{ scene, knownBy }` 형태로 «A는 알고 B는 모르는» 상태를 쓸 수 있습니다 — 모르는 인물은 `(아직 모름: 지훈)`으로 표시되어 극적 아이러니가 유지됩니다.
- **살붙임 구간 상한을 설정할 수 있습니다.** `draft.sectionOutputLimit`(기본 7,000)을 낮추면 구간이 잘게 나뉘어 호출이 늘고 분량이 늘어납니다. 목표가 상한보다 작으면 살붙임이 한 번뿐이라 분량이 뼈대 수준에 갇히는데, 프롬프트의 글자 수 지시가 실측에서 듣지 않아 손댈 방법이 없었습니다.

### 변경

- **인식하지 못하는 인자를 거부합니다.** 예전에는 `storyboard-bot init` 처럼 잘못 친 명령이 조용히 무시되고 봇이 그대로 떴기 때문에, 오타인지 설정 문제인지 구분되지 않았습니다. 이제 사용법과 함께 종료 코드 1로 끝납니다.
- **설정이 없을 때의 메시지가 온보딩 안내로 바뀌었습니다.** 첫 실행은 오류가 아니므로 `storyboard-bot setup` 을 안내합니다. 설정 파일이 있는데 깨진 경우는 종전처럼 ERROR 로그로 남습니다.
- **최종 검수가 장 단위로 나뉘어 돕니다.** 이전에는 전권을 한 호출에 넣어, 2~3권 분량이면 모델의 주의가 뒤로 갈수록 옅어져 한복판의 모순이 사실상 검출되지 않았습니다. 이제 장마다 연속성·비평을 한 쌍씩 부르고 앞 장의 요약을 함께 실어 장 경계를 넘는 모순도 봅니다. 긴 컨텍스트 모델에 기대지 않으므로 제공자를 바꿔도 검수 품질이 그대로입니다.

### 고침

- **사건이 적은 씬의 초안이 목표 분량에 한참 못 미치던 문제를 고쳤습니다.** 뼈대 프롬프트가 «묘사를 넣지 마라»와 «목표 N자»를 동시에 걸어 두 지시가 충돌했고, 어느 쪽을 택할지는 모델 성향에 달렸습니다. 실측에서 목표의 1/3만 쓰고 살붙임이 9배 확장을 떠안아 최종 분량이 목표 절반에도 못 미쳤습니다. 이제 분량을 묘사가 아니라 사건의 밀도로 채우도록 우선순위를 못박고, 뼈대가 목표의 절반에도 못 미치면 사유를 실어 한 번 더 부릅니다.
- **살붙임이 분량을 되풀이로 채우지 않습니다.** 재료가 바닥나면 모델이 분량을 채울 수단은 되풀이뿐이라, 앞 구간이 쪼개 쓴 대사를 다음 구간이 통째로 다시 쓰고 마무리 동작을 두 번 넣는 일이 있었습니다. 기존 검사는 구간 전체를 다시 쓴 경우만 잡아 이런 작은 되풀이는 통과했습니다. 이제 되풀이를 위반으로 잡아, 되풀이로 채운 긴 판보다 짧고 깨끗한 판이 남습니다.
- **긴 대사를 여러 문장으로 쪼개도 «대사가 사라졌다»고 오판하지 않습니다.** 조각을 하나씩 원문과 견주다 보니 쪼갠 대사가 각각 임계값에 못 미쳐 소실로 잡혔고, 재시도를 두 번 태우고 경고를 초안에 남겼습니다. 이제 이어진 조각을 합쳐서도 견줍니다.
- **앞 장을 고치면 그 장의 요약이 낡음으로 표시됩니다.** 요약이 어느 초안에서 나왔는지 기록이 없어, 앞 장을 재작성해도 요약은 옛 텍스트로 굳은 채 폐기된 판본의 줄거리가 다음 장 프롬프트로 계속 주입됐습니다. 표시된 장은 지우지 않고 프롬프트에서만 빠지며, 생성 경고와 `doctor`가 다시 요약할 장을 알려 줍니다.
- **앞 씬을 다시 쓸 때 뒤 씬의 정보가 새지 않습니다.** 인물·배경 기억이 카드 해시만 보고 재사용돼, 씬 30까지 진화한 기억이 씬 5를 다시 만드는 자리에 그대로 쓰였습니다. 이제 지금 만드는 씬보다 뒤에서 갱신된 기억은 새로 뽑습니다 — 원장에는 있던 되감기가 카드 기억에만 없던 비대칭을 맞췄습니다.

### 제거

- **CLI 가 0.8 이전의 `cli.json`·`cli-secrets.json` 을 더 이상 읽지 않습니다.** 공용 `~/.storyboard/config.json`·`secrets.json` 으로 이름을 바꾸거나 `storyboard setup` 을 다시 실행하세요.

## [0.8.8] - 2026-09-07

### 추가

- **시점을 씬·장마다 바꿀 수 있습니다.** 작품 계약의 시점이 1인칭 회고·2인칭을 포함해 다섯 값으로 늘었고, 시점에 이름을 붙인 **서술자 카드**(`narrator/*.card`)를 만들면 씬 카드의 `narrator`나 `chapters.yaml` 장의 `narrator`로 그 시점을 고를 수 있습니다. 서술자는 인칭·지식 경계(목격 범위/전지/회고)·시제·목소리를 가지며, 해석 순서는 씬 > 장 > 프로젝트입니다. 서술자 카드를 만들지 않으면 종전처럼 계약의 시점 하나가 모든 씬에 적용됩니다. 씬의 서술자·연속성 줄기·시점 인물은 초안 캐시와 원장 감사의 입력에 포함되므로, 시점만 바꿔도 초안이 다시 만들어지고 그 씬이 남긴 이야기 상태는 낡음으로 표시됩니다.
- **구성을 고르면 옴니버스·시점 교차·액자식으로 씁니다.** 작품 계약의 **구성**에서 고르면 연속성 줄기와 서술자 카드를 프리셋이 만들고, 아웃라인 생성이 그 구성에 맞는 지시(편마다 독립된 결말, 장별 서술자 배정, 외화의 앞뒤 배치)를 받습니다. 옴니버스에서는 이야기 상태 원장·장 요약·직전 씬 맥락·인물 기억이 편 안에서만 이어지고 캐넌만 공유됩니다.
- **1인칭·제한 시점이 화자가 모르는 사실을 서술하지 않습니다.** 이야기 상태 원장의 각 항목에 그 사실이 확립된 씬의 인물이 목격자로 기록되고, 목격 범위 서술자에게는 초점 인물이 목격하지 않은 항목이 프롬프트에서 빠집니다. 검수에도 시점 이탈과 서술자 목소리 이탈 항목이 더해졌습니다.
- **CLI에 `narrator` 명령과 `scene show`가 생겼습니다.** `storyboard narrator add|list|show|remove`로 서술자를 다루고, `storyboard init --composition omnibus --episodes 4`처럼 구성을 고를 수 있습니다. `scene show <stem>`은 그 씬이 어떤 시점·줄기로 생성될지 해석된 결과를 보여 주고, `doctor`는 카드 없는 서술자 참조와 계약에 없는 줄기를 미리 잡습니다.
- **`doctor`가 읽지 못한 원장 줄을 알려 줍니다.** 손으로 고치다 `- [2!!] …`처럼 태그가 깨지면 그 항목은 시점 필터와 낡음 판정을 받지 못한 채 조용히 남습니다. 이제 줄 수와 예시를 보고하며, 줄기별 원장까지 함께 봅니다.
- **`install.sh`가 이미 내려받은 tarball로 설치할 수 있습니다.** `--from <디렉터리>`를 주면 그 디렉터리의 `storyboard-cli`·`storyboard-bot` tarball을 씁니다. 저장소가 비공개인 동안 스크립트가 릴리스 자산을 인증 없이 받으려다 404로 멈추던 상황에서, 자산을 손으로 내려받은 뒤 설치할 수 있습니다. 같은 디렉터리에 `SHA256SUMS`가 있으면 검증하고, 없으면 경고한 뒤 진행합니다.

## [0.8.7] - 2026-09-07

### 추가

- **`install.sh` 한 줄로 CLI와 텔레그램 봇이 함께 설치됩니다.** 이전에는 CLI만 설치되고 봇은 tarball을 손으로 풀어야 했으며, 풀어도 번들 밖 의존인 `better-sqlite3`가 없어 실행이 실패했습니다. 이제 두 tarball을 체크섬 검증 후 각각 풀고 봇의 런타임 의존을 설치한 뒤 `storyboard`·`storyboard-bot` 두 실행 파일을 링크합니다. 설치 후 `storyboard-bot setup`으로 이어 가세요. Node 20 이상과 npm이 필요합니다.

### 고침

- **고친 카드·씬이 이야기 상태 원장에 반영됩니다.** 원장 항목이 어느 씬의 어떤 입력에서 나왔는지 함께 기록해, 카드나 씬을 고쳐 놓고 그 씬을 다시 만들지 않으면 그 항목을 낡음으로 판정합니다. 앞 씬을 다시 만들면 뒤 씬의 항목도 같이 낡습니다(되감기). 낡은 항목은 원장에서 지우지 않고 `- [22!] 내용`으로 표시만 하고 프롬프트에서 빼며, 생성 결과 경고와 `storyboard doctor`가 어느 씬을 다시 만들어야 하는지 알려 줍니다. 그 씬을 다시 만들면 표시가 사라집니다. 0.8 이전 원장에는 대조할 기록이 없어 낡음으로 몰지 않고, `storyboard init --repair`가 지금의 카드·씬으로 봉인합니다.
- **풀어 놓은 tarball에서 `npm install`이 404로 실패하던 문제를 고쳤습니다.** 배포본이 소스 매니페스트를 그대로 실어 레지스트리에 없는 `@storyboard/*` 워크스페이스 의존이 들어가 있었습니다. 이제 각 tarball은 번들이 실제로 필요로 하는 외부 의존만 적은 런타임 매니페스트를 싣습니다. **0.8.5 이하 버전은 `install.sh`가 거부하므로**, 예전 릴리스를 쓰신다면 이 버전 이상으로 올려 주세요.

## [0.8.6] - 2026-09-07

### 추가

- **씬 카드에 사건 비트(`beats`)가 생기고, 초안 생성 전에 자동으로 전개됩니다.** 초안 분량은 씬 카드의 사건 밀도가 정하는데, 창작자가 쓴 짧은 요약만으로는 목표 분량에 못 미치는 일이 잦았습니다. 이제 `scene generate`는 카드에 `beats`가 없으면 카드 재료·근거 사실·요약을 바탕으로 비트를 먼저 뽑아 카드에 쓴 뒤(요약이 있으면 그 범위 안에서만) 초안을 씁니다. 비트 수는 `max(draft.minBeats, ceil(targetWordCount / draft.charsPerBeat))`(기본 5 · 1,500자)이고, `draft.autoBeats`를 끄면 이 단계를 건너뜁니다. 미리 검수하려면 CLI `storyboard scene beats <stem> | --all [--dry-run] [--force]`, 확장 «Storyboard: 씬 비트 전개 (현재 씬)», 봇 `/scene beats <stem> [force]`를 쓰세요 — 이미 있는 비트는 `--force`/`force`로만 다시 뽑습니다. 초안 프롬프트에서는 `beats`가 `summary`보다 우선합니다.
- **씬 요약이 `scene/<stem>.summary.md` 파일로 옮겨 갑니다.** 카드의 `summary`에는 파일명만 남고 산문은 옆 파일에 둡니다(창작자가 쓴 사건과 기계가 펼친 비트를 구별하기 위해서입니다). 카드 에디터의 Summary 칸과 봇 `/scene edit|append`는 이 파일을 읽고 씁니다. 기존 인라인 요약은 `storyboard scene migrate`가 파일로 뽑아 주고, `doctor`가 인라인 요약이 남은 씬 수와 비트 없는 씬 수를 경고합니다. `scene complete`가 새로 제안하는 씬은 당분간 인라인 요약으로 나오며, 첫 편집이나 `scene migrate`에서 파일로 옮겨집니다.

## [0.8.5] - 2026-09-07

### 추가

- **CLI가 0.8 이전 워크스페이스를 알아보고 고쳐 줍니다.** `doctor`가 빠진 디렉터리(`draft/`·`scene/`), 구형 `scene/*.txt`, 자동 생성 시드의 플레이스홀더 요약, 오래된 `.gitignore` 블록, 읽을 수 없는 씬 카드를 각각 한 줄씩 보고하고 고치는 명령을 함께 알려 줍니다. `storyboard init --repair`는 이미 있는 워크스페이스의 디렉터리와 `.gitignore`만 최신으로 맞추고 작품 계약은 건드리지 않으며, 그냥 `init`은 기존 워크스페이스를 거부합니다. `scene migrate`는 `.txt` 변환에 더해 플레이스홀더 줄을 지우되, 창작자가 그 위에 써 둔 요약은 그대로 둡니다.
- **`doctor`가 구독형 CLI 제공자의 로그인 상태를 확인합니다.** 실행 파일이 있어도 로그아웃 상태면 실패로 보고합니다. 이 검사는 제공자 CLI를 실제로 한 번 띄우므로 최대 15초쯤 걸릴 수 있습니다.
- **`scene generate`가 초안의 경고를 보여 줍니다.** 분량 미달 같은 경고가 stderr에 찍히고 `--json`의 `data.warnings`에도 실립니다(종료 코드는 0 유지). `--verbose`를 주면 뼈대·살붙임·검수 같은 파이프라인 단계가 진행되는 대로 stderr에 남습니다.

### 고침

- **claude-code 제공자가 소설가로만 동작합니다.** 도구를 끄고 시스템 프롬프트를 통째로 바꿔, 초안 본문에 "I'll write the scene…" 같은 에이전트 말투가 섞이지 않습니다.
- **claude-code 생성 실패 사유가 메시지에 드러납니다.** 이전에는 "비정상 종료했습니다 (exit 1)"만 보였지만, 이제 한도 초과·인증 문제 등 CLI가 낸 사유가 함께 표시되고 `doctor`의 로그인 실패도 원인을 덧붙입니다.
- **`.gitignore` 블록이 세 앱에서 한 정의를 씁니다.** 마커만 있고 항목이 빠진 오래된 블록에 빠진 줄(`manuscript/` 등)이 이어 붙습니다.
- **씬 카드 하나가 깨져도 `doctor`와 `scene migrate`가 멈추지 않습니다.** YAML이 어긋난 카드가 있으면 `doctor`는 진단 자체를 못 냈고, `scene migrate`는 `.txt` 변환까지만 하고 예외로 죽어 절반만 처리된 상태로 남았습니다. 이제 두 명령 모두 읽지 못한 파일 이름을 따로 보고하고 나머지는 계속 처리합니다.
- **생성 직후 경고를 먼저 보여 줍니다.** `scene generate`의 분량 경고가 검수 재작성이 끝난 뒤에 찍혀, 이미 고쳐진 문제를 다시 알리던 문제입니다.

## [0.8.4] - 2026-09-07

### 추가

- **xAI Grok을 제공자로 고를 수 있습니다.** `grok` 제공자는 xAI의 OpenAI 호환 API(`api.x.ai`)를 쓰며, 키는 다른 API 제공자처럼 `~/.storyboard/secrets.json`에 넣습니다. 모델 카탈로그는 `grok-4.6`·`grok-4.5`·`grok-4.3`이고 단가표가 있어 달러 비용이 함께 표시됩니다.
- **Gemini CLI를 제공자로 고를 수 있습니다.** `gemini-cli` 제공자는 Claude Code·Codex처럼 설치된 `gemini` 명령을 헤드리스로 실행해 Google 계정 로그인만으로 생성합니다. 모델은 `flash`·`pro` 별칭이나 `gemini-2.5-pro` 같은 id를 쓰고, 사용량은 CLI의 JSON `stats`에서 읽으며, 사용 한도에 걸리면 다른 CLI와 같은 폴백 규칙을 따릅니다. 연결 테스트는 설치 여부만 확인하고 로그인 안 됨은 첫 생성에서 안내합니다.
- **봇이 API 키 제공자도 씁니다.** 이전에는 봇이 CLI 제공자(claude-code·codex·mock)만 허용해 공통 설정이 OpenAI·Claude·Gemini를 가리키면 부팅을 거부했습니다. 이제 봇도 공통 `~/.storyboard/secrets.json`의 키를 읽어 모든 제공자로 생성하며, `/doctor`가 사용 중인 제공자의 키 유무를 보고하고 `storyboard-bot setup`은 전체 제공자를 제시합니다.

### 고침

- **사용 한도에 걸린 CLI 제공자를 계속 다시 부르지 않습니다.** 이전에는 한도 전환 상태가 호출마다 초기화돼, 한 번 한도에 걸린 뒤에도 남은 모든 작업이 그 CLI를 다시 실행해 거절당한 다음에야 대체 제공자로 넘어갔습니다. 장편 한 편이면 수백 번의 헛된 실행이었습니다. 이제 전환은 실행이 끝날 때까지 유지됩니다. 함께, 몇 초면 풀리는 분당 요청 제한(`rate limit`·`429`)은 한도 소진으로 보지 않습니다. 예전에는 순간적인 제한 하나로 원고의 나머지가 통째로 다른 모델로 넘어갈 수 있었습니다.
- **구독 CLI 사용량이 섞인 합계에 «구독 CLI 사용량 제외»를 붙입니다.** 봇의 `/usage`와 상태 보고가 단가표 없는 제공자의 토큰을 $0으로 더해, 실제 청구액보다 적은 금액을 정확한 수치처럼 보여 주던 문제입니다.
- **Gemini CLI가 성공한 응답을 버리지 않습니다.** 정상 실행 중 stderr에 인증 관련 경고가 섞이면 응답을 버리고 로그인 오류로 처리하던 문제를 고쳤습니다.
- **키 없는 제공자로는 생성 작업을 큐에 넣지 않습니다.** 봇이 API 키 제공자를 쓰게 되면서, 키가 없으면 작업이 실행될 때까지 기다렸다가 실패하던 것을 명령 즉시 거절하도록 했습니다.
- **봇 설정 마법사가 제공자를 못 정하면 중단합니다.** 잘못된 답을 세 번 하면 조용히 `codex`로 확정하던 것을, 다른 단계처럼 중단하도록 했습니다.
- **`bot.json`의 `"sceneDraft": "codex"` 형태를 실제로 적용합니다.** 스키마는 통과하지만 엔진이 읽지 못해 해당 작업이 조용히 기본 제공자로 떨어지던 문제입니다.
- **API 키 저장 명령이 키가 필요한 제공자만 제시합니다.** `ollama`나 CLI 제공자에 키를 저장해도 아무 효과가 없었습니다.

## [0.8.3] - 2026-09-05

### 추가

- **전권 최종 검사 결과가 원고에 되먹여집니다.** 이전에는 장거리 모순을 찾아내고도 `REVIEW.md` 리포트로만 남아, 어느 초안을 고쳐야 하는지조차 알 수 없었습니다. 이제 검사가 각 이슈에 씬을 지목하고, 심각한 이슈가 있는 씬만 한 번 재작성한 뒤 전권을 다시 검사해 리포트를 갱신합니다. 씬을 특정하지 못한 이슈는 버리지 않고 리포트에 건수로 남습니다. 재작성 전에 대상 씬을 확인하고 승인받는 실행 모드도 추가됐습니다(CLI는 무인이라 그대로 자동 진행).

### 변경

- **장을 하나 끝낼 때마다 «지금까지의 줄거리»를 갱신합니다.** 이전에는 롤링 요약이 파이프라인 맨 마지막에만 만들어져, 원클릭 첫 실행 내내 요약 파일이 비어 있었습니다. 그래서 뒤 장이 앞 이야기를 모른 채 직전 초안 꼬리 1000자만 보고 쓰였습니다. 주입 예산도 2,000자에서 8,000자로 늘렸고, 넘칠 때는 앞부분을 잘라 내는 대신 오래된 장부터 제목과 한 줄로 압축하므로 도입부에 심어 둔 복선이 사라지지 않습니다.
- **AI가 만든 기억이 `.storyboard/memory/`로 옮겨 가고 git 추적 대상이 됩니다.** 이야기 상태 원장·인물/배경 기억·대사 기록·장별 요약은 커밋된 입력으로 다시 만들어 낼 수 없는데도 무시 대상인 `.storyboard/cache/`에 있어 버전 관리 밖에 놓여 있었습니다. **기존 워크스페이스는 처음 열 때 자동으로 한 번 이관되므로** 따로 하실 일은 없습니다.

## [0.8.2] - 2026-09-04

### 변경 (호환성 주의)

- **설정과 API 키가 세 앱이 함께 쓰는 `~/.storyboard/`로 옮겨 갑니다.** 이전에는 확장은 VSCode `settings.json`과 비밀 저장소, CLI는 `cli.json`, 봇은 `bot.json`에 따로 두어 같은 키를 세 번 넣어야 했습니다. 이제 공통 설정은 `~/.storyboard/config.json`, 작품별 설정은 `<워크스페이스>/.storyboard/config.json`, 키는 `~/.storyboard/secrets.json`(0600) 한 곳입니다. **확장은 처음 켤 때 기존 `storyboard.*` 설정과 저장된 키를 자동으로 한 번 옮기고 VSCode 쪽에서 지웁니다** — 별도 작업은 필요 없지만, 이후로는 VSCode 설정 화면에 `storyboard.*` 항목이 나오지 않고 설정 패널에서 바꿉니다. CLI의 옛 `cli.json`·`cli-secrets.json`과 봇의 `bot.json` AI 블록은 경고와 함께 당분간 계속 읽힙니다.
- **AI 제공자를 고르기 전에는 생성이 거부됩니다.** 이전에는 기본값이 `mock`이라 설치 직후 아무것도 고르지 않아도 가짜 초안이 만들어졌습니다. 이제 제공자가 없으면 오류로 멈추고, 확장은 «제공자 선택» 버튼을, CLI는 `storyboard setup`을 안내합니다. 인라인 완성·문법·연속성 같은 배경 기능은 오류 없이 조용히 건너뜁니다.

### 추가

- **CLI가 대화형 화면을 갖습니다.** 인자 없이 `storyboard`를 실행하면(또는 `storyboard tui`) 워크스페이스와 제공자를 머리에 띄운 화면이 열려, 명령을 이어서 입력하고 진행과 결과를 그 자리에서 봅니다. 접두 제안(Tab)·히스토리·`/help` `/doctor` `/setup` `/clear` `/quit`를 지원하며, 터미널이 아니면 열리지 않으므로 에이전트가 갇힐 일은 없습니다.
- **셸 탭 완성이 붙습니다.** `storyboard completion <zsh|bash|fish>`가 등록 스크립트를 내며, 명령·옵션은 물론 워크스페이스의 씬·카드 이름까지 완성합니다. `install.sh`로 설치하면 쓰는 셸의 rc 파일에 자동 등록됩니다.
- **`setup`·`doctor`·`config` verb가 생겼습니다.** `setup`은 새 기기에서 제공자와 키를 잡아 주고, `doctor`는 홈·설정·제공자·키·실행 파일·워크스페이스를 점검해 **고치는 명령까지** 알려 주며, `config show|set`은 값과 그 값이 어디서 왔는지 보여 줍니다.
- **`--help`가 읽을 만해졌습니다.** 30개 명령이 7개 그룹과 예시로 정리되고, `<명령> --help`로 개별 도움말을, `-h`/`-v`도 받습니다. 오타는 가까운 명령을 제안합니다. 인자 없이 실행해도 더 이상 오류로 끝나지 않습니다.
- **오래 걸리는 8개 명령이 진행 상황을 보여 줍니다.** 씬 시드 생성과 구형 씬 변환은 N/M 진행률까지 표시합니다.
- **실패 알림에 다음 행동 버튼이 붙습니다.** API 키가 없으면 «설정 열기», 제공자가 없으면 «제공자 선택»이 토스트에 함께 뜹니다(13곳). 빈 사이드바도 안내 문장 대신 실행 버튼을 보여 줍니다.
- **설정 패널이 저장 결과와 값의 출처를 알려 줍니다.** 저장하면 «이 작품 / 공통 설정 중 어디에 저장됐는지»를 알리고, 값마다 출처 배지가 붙습니다. 새 «옵션» 탭에서 생성·검수 스위치를 바꿀 수 있고, 상태바에서 현재 기본 제공자와 모델을 볼 수 있습니다.
- **봇도 공통 설정을 읽습니다.** 확장이나 CLI에서 고른 제공자가 봇에 그대로 적용되고, `/doctor`가 실제로 쓰이는 제공자를 보고합니다.

### 고침

- **구독형 제공자의 비용 배지가 항상 $0.00으로 나오던 문제를 고쳤습니다.** `codex`·`claude-code`처럼 단가가 없는 제공자는 «12.3k 토큰»으로, 단가가 있는 제공자는 «$0.42 · 12.3k»로, 기록이 없으면 «—»로 구분해 표시합니다. Studio 대화의 사용량도 이제 장부에 잡힙니다.
- **인물·배경·씬 카드를 열면 Studio 채팅이 비활성화되던 문제를 고쳤습니다.** `.card`는 커스텀 에디터라 활성 텍스트 에디터로 잡히지 않았습니다.

## [0.8.1] - 2026-09-04

### 고침

- **긴 씬의 뒷부분이 앞부분을 통째로 되풀이하던 문제를 고쳤습니다.** 32씬을 생성했을 때 11개 씬에서 후반이 전반의 복사본이 되고 한 씬은 18%가 중복이었습니다. 원인이 세 겹이라 세 곳을 모두 막았습니다 — 뼈대가 같은 대사 묶음을 두 번 적으면 사유를 붙여 다시 부르고, 살붙임이 직전 구간을 다시 쓰면 반려하며, 문장을 새로 지어 축자 비교를 피해 간 판도 대사 순서로 잡아냅니다.
- **마지막 구간에 재료 없이 분량만 요구하던 문제를 고쳤습니다.** 구간 예산을 균등하게 나누지 않고 뼈대 조각의 실제 길이에 비례해 배분하므로, 얇은 조각이 분량을 채우려 앞 내용을 되풀이할 이유가 없어집니다.
- **아웃라인이 정한 씬별 목적·갈등·반전이 초안 생성에 전달됩니다.** 이전에는 씬 시드로 만든 씬을 그대로 생성하면 모델이 받는 사건 재료가 "자동 생성된 씬 시드입니다" 한 줄뿐이었습니다.
- **모델이 모르는 값을 빈 문자열로 적어 보내면 카드 구성 결과가 통째로 버려지던 문제를 고쳤습니다.** 빈 선택 필드는 적지 않은 것으로 읽습니다.
- **한글 이름으로 카드를 만들 때 이름과 무관한 `new-card`, `new-card-2` id가 붙던 문제를 고쳤습니다.** 이제 쓸 수 있는 id를 만들 수 없으면 조용히 짐작하는 대신 `--id`를 요구합니다.

### 변경

- **새 워크스페이스에서 `draft/`가 git 추적 대상이 됩니다.** 초고는 사람이 읽는 결과물인데 이력이 남지 않아 되돌아갈 지점이 없었습니다. 초고에서 다시 만들 수 있는 `.draft/`·`manuscript/`·`.storyboard/cache/`는 그대로 제외됩니다. 기존 워크스페이스의 `.gitignore`는 바뀌지 않으니, 초고를 추적하시려면 `draft/` 줄을 직접 지우세요.

### 추가

- **작품 계약으로 장·씬 개수를 고정할 수 있습니다.** `chapterCount`와 `scenesPerChapter`를 적어 두면 아웃라인을 다시 만들어도 구성이 흔들리지 않습니다. 이전에는 설명문에 자연어로 부탁하는 수밖에 없었고 값이 어디에도 기록되지 않았습니다.
- **CLI가 작품 계약을 입력으로 받습니다.** `storyboard init`에 계약 플래그와 `--from <json>`을 줄 수 있고, `storyboard project set`으로 나중에 고칠 수 있습니다. 이전에는 `init`이 만든 `project.json`에 `setting`이 없어 CLI만으로는 아웃라인 생성까지 갈 수 없었습니다.
- **CLI의 `cards build`와 `scene complete`가 파일까지 씁니다.** 제안 JSON만 보려면 `--dry-run`을 쓰세요. 완결 씬은 이미 있는 파일을 건너뜁니다.

## [0.8.0] - 2026-09-03

### 추가

- **`storyboard` 명령줄 앱이 추가되었습니다.** 에디터 없이 터미널에서 워크스페이스를 만들고, 카드·씬을 만들고 고치고, 초안을 생성·재생성하고, 세 가지 검사(연속성·문법·캐넌)를 돌리고, 원고를 내보낼 수 있습니다. 결과는 표준 출력으로, 진행 상황과 경고는 표준 오류로 나가며 종료 코드로 성패를 알리므로 다른 AI 에이전트가 그대로 몰아 쓸 수 있습니다. 설정은 `~/.storyboard/cli.json`(워크스페이스별로 `.storyboard/cli.json`이 덮어씀), 키는 `~/.storyboard/cli-secrets.json`에 0600으로 보관합니다.
- **한 태그가 세 가지 산출물을 냅니다.** 확장 VSIX, 봇 tarball, CLI tarball이 체크섬과 함께 같은 릴리스에 올라가고, `scripts/install.sh`로 CLI를 한 줄에 설치할 수 있습니다.

### 변경 (호환성 주의)

- **확장 ID가 `maroomir.storyboard`에서 `maroomir.storyboard-vscode`로 바뀝니다.** VSCode는 이전 확장과 별개로 취급하므로, 기존에 설치해 두셨다면 옛 확장을 지우고 새 VSIX를 설치해야 합니다. 명령 이름(`Storyboard: …`)과 명령 ID(`storyboard.*`), 설정 키(`storyboard.*`), 워크스페이스 파일 형식은 그대로입니다. CLI 실행 파일이 `storyboard` 이름을 갖게 되면서 확장 쪽 이름을 구분한 결과입니다.

### 제거

- **텔레그램 봇 관련 기능이 확장에서 모두 빠졌습니다.** 상태바 관찰, 설정 패널의 «텔레그램 봇» 탭, `Storyboard: 텔레그램 봇 설정…`·`설정 파일 열기`·`재시작`·`대시보드 열기` 네 명령이 사라집니다. 봇은 이제 완전히 독립한 앱이라 설정과 실행을 스스로 관리하며(`node dist/index.js setup`), 확장은 봇의 존재를 알지 않습니다.

### 고침

- **봇이 초안을 생성할 때 캐넌과 이야기 상태를 빠뜨리던 문제를 고쳤습니다.** 봇은 이제 확장·CLI와 같은 생성 파이프라인을 그대로 돌리므로, 어느 앱에서 만들든 초안 품질이 같습니다.
- **봇의 원고 조립이 자체 구현 대신 공용 파이프라인을 씁니다.**
- **내용이 빈 기획으로 아웃라인을 만들려 하면 봇이 거부합니다.** 이전에는 빈 아웃라인을 만들어 두고 성공으로 처리했습니다.
- **커밋을 우회하는 봇 쓰기를 막았습니다.** 추적 대상 파일은 반드시 뮤테이트 게이트를 거쳐야 하며, 그렇지 않은 경로는 거부됩니다.
- **CLI 제공자의 사용량 한도 대체 동작을 되살렸습니다.**
- **파일 수정 시각 계약이 앱마다 달라 봇 초안 게이트가 잘못 판정하던 문제를 고쳤습니다.**

### 내부

- 저장소가 npm workspaces 모노레포로 바뀌었습니다: `apps/vscode`·`apps/bot`·`apps/cli` 세 앱이 `packages/story-engine`·`story-format`·`story-ai`·`story-git`을 공유하고, 버전은 루트 `package.json` 하나만 두고 `npm run version:sync`로 내려보냅니다. 세 앱이 같은 코덱으로 쓰기 때문에 어디서 고친 카드든 바이트 단위로 같게 저장됩니다.
- 앱마다 계층 방향과 순환 참조를 검사하는 아키텍처 스크립트가 붙었고, 임포트는 앱 안에서는 `@/`, 패키지 안에서는 Node 서브패스(`#engine/` 등) 별칭으로 통일했습니다.

## [0.7.3] - 2026-08-31

### 추가

- **Studio 대화가 도구를 씁니다.** 초안 대화에서 에이전트가 필요하다고 판단하면 연속성 검사·문법 검사·구간 확장·축소·카드 보충을 스스로 불러 결과를 반영합니다. 검사 결과는 대화 요약과 함께 에디터에도 물결선으로 표시되고, 변환 결과는 초벌을 에이전트가 다듬어 언제나 제안 카드(diff → 승인)를 거칩니다. 도구는 한 턴에 최대 2번까지만 돕니다.
- **`/`로 도구를 직접 지정할 수 있습니다.** 입력창에 `/`를 치면 대상에 맞는 도구 목록이 뜨고, 고르면 칩으로 박혀 그 도구를 반드시 쓰게 됩니다. 뒤에 자연어 주문을 이어 적거나 그냥 보내도 됩니다.
- **카드 대화에도 도구가 생겼습니다.** 인물·배경 카드에서 `/collect`(등장 초안에서 더할 정보 수집), `/audit`(다른 자료와 어긋나는지 검사), `/relations`(끊어진 참조·일방향 관계 점검 — AI 호출 없음)를 쓸 수 있고, 씬 카드에서는 `/audit`·`/relations`를 쓸 수 있습니다.
- **`/update`로 설명문만 붙여넣어 카드를 만들거나 채웁니다.** 인물/배경을 판별해 없는 카드는 새로 만들어 열고, 있는 카드는 그 카드를 연 뒤 "이 설명으로 채워줘" 지시를 입력창에 채워 둡니다. 파일을 열지 않은 상태에서도 쓸 수 있습니다.

### 변경

- **씬 카드 대화가 `characters`·`location`도 채웁니다.** 긴 설명문을 붙여넣으면 필드로 구조화해 한 번에 제안하며, 실제로 존재하는 카드 id만 들어갑니다 — 없는 카드를 가리키는 제안은 적용 단계에서 거부됩니다.

## [0.7.2] - 2026-08-31

### 변경

- **Studio 패널이 대상과 대화하는 방식으로 바뀌었습니다.** 인물·배경 카드나 씬·초안을 열고 고치고 싶은 내용을 자연어로 적으면, 지시가 모호할 때는 되묻고 정해지면 수정안을 제안합니다. 제안은 무엇이 바뀌는지와 기존 설정과 충돌하는지를 함께 보여 주며, diff로 확인하고 **승인**해야 파일에 반영됩니다. 이전의 슬래시 명령과 12개 작업 버튼은 없어졌고, 초안 생성·재생성, 문법·연속성 검사, 이야기 완결, 씬 기반 카드 구성 같은 파이프라인 작업은 명령 팔레트와 뷰 헤더에서 그대로 실행합니다.
- **대화가 대상별로 남습니다.** 카드나 씬을 열면 그 대상의 마지막 대화가 복원되고, 헤더의 대화 기록에서 지난 대화를 골라 이어갈 수 있습니다. 다른 파일을 열면 대화창도 그 대상의 것으로 바뀝니다. 수정이 실제로 적용된 대화는 계속 보관되고, 아무것도 바꾸지 않은 대화만 대상당 10개까지 남습니다. 카드 이름을 바꾸면 대화도 함께 따라갑니다.

### 추가

- **인물 카드에 변화 곡선을 적을 수 있습니다.** 이야기를 거치며 인물이 어떻게 변하는지 단계·요약·씬 참조로 `arc`에 정리하도록 대화로 지시할 수 있습니다. `description`·`traits`·`voice`·`desire`에는 이야기 내내 참인 상태만 두고 변화는 `arc`로 분리되므로, 뒷부분 줄거리가 앞 씬 생성에 새어 들어가지 않습니다.
- **한 곳을 고치면 함께 손볼 대상을 짚어 줍니다.** 배경을 바꿔 인물이나 씬도 손봐야 하면 제안 아래에 그 대상 버튼이 나오고, 누르면 파일을 열어 새 대화를 시작하면서 넘겨받은 지시문을 입력창에 채웁니다. 제안을 승인하면 그 대상에 "넘어온 작업"으로 기록되어, 나중에 그 카드나 씬을 열었을 때 목록으로 보입니다.
- **정합성 검사를 끌 수 있습니다.** `storyboard.studio.validation`을 끄면 제안마다 추가로 부르던 AI 검사를 건너뜁니다.

### 고침

- **모델 응답 끝에 붙은 여분의 문자 때문에 결과가 통째로 버려지던 문제를 고쳤습니다.** JSON 응답을 받는 모든 작업에 적용됩니다.
- **다른 도구가 먼저 고친 파일은 덮어쓰지 않습니다.** 대화로 만든 수정안을 승인할 때, 그 사이 편집기나 텔레그램 봇이 파일을 바꿨다면 적용을 거부하고 다시 요청하라고 알립니다. 초안 구간 수정은 고칠 자리의 원문까지 대조해, 위치가 어긋나면 적용하지 않습니다.
- **대화로 초안을 고칠 때도 이전 판본을 보관합니다.** `storyboard.draft.keepHistory`가 켜져 있으면 적용 직전 초안을 `.draft`에 남깁니다.

## [0.7.1] - 2026-08-31

### 추가

- **인물이 앞서 한 말을 말투 기준으로 씁니다.** 초안의 대사에 화자를 붙여 기록해 두고, 다음 씬의 대사를 다듬을 때 그 인물이 실제로 했던 말 몇 개를 참고로 넣습니다. 씬이 쌓일수록 같은 인물의 어미·호칭이 덜 흔들립니다.
- **같은 장소가 다시 나오면 배경 묘사를 갱신합니다.** 직전에 그 장소가 나온 씬의 원고를 참고해, 거기 놓여 있던 사물과 구조가 다음 씬의 묘사에도 남습니다. 이전에는 첫 등장 씬에서 만든 묘사가 카드를 고칠 때까지 그대로였습니다.

### 고침

- **다른 도구로 쓴 초안을 덮어쓰기 전에 보관합니다.** 텔레그램 봇으로 생성했거나 직접 고친 초안 위에서 생성을 실행하면, 히스토리 설정과 무관하게 이전 판본을 `.draft`에 남긴 뒤 덮어씁니다. 이전에는 되돌릴 방법 없이 사라졌습니다.
- **작품이 길어져도 앞부분 설정을 기억합니다.** 이야기 상태 기록이 일정 개수를 넘으면 오래된 항목부터 버리던 것을, 전부 보관하고 프롬프트에 실을 때만 골라 넣도록 바꿨습니다. 1막에서 심은 사실도 3막 씬에 그 이름이 나오면 다시 참고됩니다.
- 파일을 쓰는 도중 중단돼도 잘린 파일이 남지 않습니다.
- 검수 재작성을 거쳐도 생성 단계가 남긴 경고가 초안 헤더에 그대로 남습니다. 이전에는 재작성 시 사라졌습니다.

## [0.7.0] - 2026-08-30

### 변경

- **씬 생성 방식을 뼈대 우선으로 바꿨습니다.** 씬을 비트로 쪼개 따로 쓰던 방식이 씬 전체의 뼈대를 한 번에 쓰고 → 대사의 말투를 다듬고 → 구간별로 살을 붙이는 4단 흐름으로 바뀌었습니다. 사건·등장·종료 지점이 한 문맥에서 확정되므로 같은 인물이 두 번 처음 등장하거나 이미 벌어진 일이 다시 벌어지는 연속성 결함이 줄어듭니다. 같은 씬 카드로 생성해도 이전 판본과 원고의 성격이 달라집니다.
- 살붙임 결과를 AI 없이 기계로 검증합니다. 뼈대에 없던 인물, 다른 문자 체계 오염, 사라진 대사, 분량 미달을 판정해 재시도하고, 재시도로도 남으면 초안 헤더의 `warnings`로 알립니다.
- 대사 다듬기 단계는 말투만 고칩니다. 대사의 개수와 순서는 뼈대 그대로 두므로, 앞 대사를 의문형으로 되풀이하기만 하는 빈 대사가 생기지 않습니다.
- 페르소나에 예시로 적힌 대사가 원고에 그대로 복사되지 않습니다.

### 추가

- **이야기 상태 원장**을 도입했습니다. 씬을 생성할 때마다 확정 사실·관계 변화·공개된 정보·반복 모티프를 `.storyboard/cache/storyState.md`에 쌓아 다음 씬 생성에 넘깁니다. 앞 씬에서 이미 밝혀진 사실이 뒷 씬에서 다시 처음 공개되는 일을 막습니다.
- **캐넌 사실에 공개 시점을 지정**할 수 있습니다. `revealFrom`에 씬 번호를 적으면 그 씬 전까지는 프롬프트에 주입되지 않아, 결말의 반전이 초반 씬에 새어 나가지 않습니다.
- **씬 카드에 종료 지점(`endState`)과 시점 인물(`povCharacter`)**을 추가했습니다. 종료 지점은 씬 폼에서 편집하며, 그 지점 뒤의 사건은 다음 씬의 몫으로 남깁니다.
- 원고에 한글이 아닌 다른 문자 체계가 섞이면 초안에서 표시합니다.
- 씬 안의 장면 전환을 원고에 표시하고, 구간을 나눌 때 그 전환 지점을 우선해서 자릅니다.

### 수정

- 감수 루프가 목표 분량에 미달한 원고에서 수정본을 모두 버리던 문제를 고쳤습니다. 원본보다 길어진 수정본까지 분량 미달로 기각돼, 지적을 고친 결과가 채택되지 않았습니다.
- 살붙임의 분량 하한을 목표의 절반에서 85%로 올려 재시도가 실제로 걸리도록 했습니다.
- 뼈대의 대사를 살붙임이 조사만 다듬어도 누락으로 잡던 문제를 고쳤습니다. 이제 유사도로 판정합니다.
- 재시도가 모두 실패하면 마지막 판이 아니라 위반이 가장 가벼운 판을, 같은 무게면 목표 분량에 가까운 판을 채택합니다.
- 목표 분량에 미달한 원고는 수동 압축이 성공할 수 없던 문제를 고쳤습니다.
- 인물을 본명뿐 아니라 작중에서 불리는 이름으로도 인식합니다.

## [0.6.4] - 2026-08-25

### 변경

- **씬 시드가 카드가 되었습니다(BREAKING).** `scene/NN-slug.txt`가 `scene/NN-slug.card`(YAML, `type: scene`)로 바뀌어 목적·갈등·반전·감정 변화·복선·필요 설정과 사건·장소·관계·시점을 구조 필드로 담습니다. 구형 `scene/*.txt`는 더 이상 읽지 않으므로 기존 워크스페이스는 아래 마이그레이션 명령을 먼저 실행해야 합니다. 프롬프트에 들어가는 `[목적]` 블록 형식은 카드에서 파생해 그대로 유지됩니다.
- 씬 사실 확정 화면을 좁은 네이티브 모달에서 QuickPick 목록으로 바꿨습니다. 사건·장소·관계·시점이 에디터 전폭으로 한 항목씩 표시되어 긴 문장이 잘리지 않고, AI가 제안한 항목에는 ✨ 표시가 붙습니다. 고치고 싶은 항목만 골라(또는 연필 버튼으로) 바로 수정할 수 있어, 네 항목을 순서대로 모두 거치던 입력 흐름이 사라졌습니다.
- 씬 카드도 카드 커스텀 에디터에서 엽니다. 씬 폼(사실 4요소·구조 필드·요약)과 씬 미리보기가 추가되었고, 수집·AI 기록 탭은 인물/배경 카드 전용으로 남습니다.
- `Storyboard: 씬을 카드로 마이그레이션`(Storyboard: Migrate Scenes to Cards) 명령을 추가했습니다. 확인 후 기존 `scene/*.txt`를 `.card`로 변환하고 원본을 지우며, 실패한 파일은 목록으로 보고합니다. `[목적]` 라벨 블록은 구조 필드로, 자유 산문은 `summary`로 무손실 매핑합니다. 텔레그램 봇에서는 `/doctor`가 남은 구형 씬을 보고하고 `/doctor migrate`가 단일 커밋으로 변환합니다.
- 씬 카드의 요약에서 비어 있는 구조 필드만 AI가 제안하는 패널을 카드 에디터에 추가했습니다. 제안을 검토한 뒤 반영하며, 이미 적어 둔 값은 덮어쓰지 않습니다.

### 수정

- GitHub Release 본문에 변경 내역 전문이 들어가던 문제를 고쳤습니다. 이제 해당 태그 버전의 섹션만 릴리스 노트로 나갑니다.

## [0.6.3] - 2026-08-23

### 변경

- Studio 패널의 첫 화면을 «지금 무대» 카드로 바꿨습니다. 파일명만 띄우던 타깃 칩 대신 씬 제목, 초안 분량·버전·최근 갱신 시점·검수 상태, 연결된 인물·배경 카드를 한 장에 모아 보여 주고, 대화 전 화면에는 그 상태에서 도출한 추천 3개를 이유와 함께 제시합니다. 명령을 실행할 때마다 다시 조회하므로 파일을 고쳐도 상태가 따라옵니다. 대화·승인·슬래시 명령 흐름은 그대로입니다.
- 설정의 태스크 탭을 오버라이드 인박스로 바꿨습니다. 24개 태스크를 모두 늘어놓는 대신 기본값을 덮어쓴 태스크만 남기고, 나머지는 검색 피커로 필요할 때 추가합니다. 탭 상단에는 기본 AI·연결 상태·오버라이드 개수를 요약하는 카드 3장이 붙고 각 카드를 누르면 해당 탭으로 이동합니다.

## [0.6.2] - 2026-08-23

### 추가

- 씬 사실 시트(grounding)를 추가했습니다. 초안 생성 직전에 사건·장소·관계·시점 네 가지를 확정해 씬 frontmatter에 남기고 대사 생성 프롬프트에 주입하므로, 추상적인 씬 시드가 비유만으로 흘러가지 않습니다. 비어 있는 항목만 AI가 제안하고 사용자가 적어 둔 값은 그대로 유지하며, 네 항목이 모두 차 있으면 AI 호출 없이 바로 생성으로 넘어갑니다. 기본은 제안을 검토한 뒤 승인하는 방식이고 `storyboard.grounding.autoApprove`를 켜면 자동 수락해 한 번의 클릭으로 생성이 끝납니다. 확정된 사실은 씬 입력 해시에 포함되므로 사실을 고치면 초안 캐시가 무효화됩니다.
- 작법 계약(craft contract)을 추가했습니다. 해설 지문 금지, 모티프·후렴 반복 상한, 상투 표현 블랙리스트, 인물 내면 요구, 기본 분량 예산을 생성·형식 프롬프트에 항상 주입합니다. 기본 계약이 내장되어 있어 별도 설정 없이 적용되며, `.storyboard/project.json`의 `setting.craftContract`로 작품별로 덮어쓸 수 있습니다.
- 초안 frontmatter에 출처 정보를 남깁니다. 생성·재작성·형식 적용 시 사용한 도구(`storyboard@<버전>`)와 프로바이더·모델이 `generator`·`providerId`·`model` 키로 기록되어, 나중에 어떤 설정으로 뽑은 원고인지 확인할 수 있습니다. 보충·압축의 diff 미리보기 경로에서도 기존 출처 정보가 보존됩니다.
- 텔레그램 봇(storygram)도 같은 흐름을 따릅니다. 초안을 공용 frontmatter 코덱으로 저장하며 `storygram@<버전>`과 프로바이더·모델을 남기고, 생성 전에 grounding을 채웁니다. 큐에 올라간 작업은 텔레그램에 승인을 물을 수 없으므로 `draft.autoGrounding`(기본 켜짐)이 채우고 커밋하는 방식으로 동작하며, 끄면 grounding을 건드리지 않습니다. 저장은 읽은 시점 해시를 기준으로 하므로 작업 도중 데스크톱에서 같은 씬을 고치면 봇이 저장을 건너뜁니다.

### 수정

- 목표 분량을 지정하지 않은 씬의 초안이 무한정 길어지던 문제를 고쳤습니다. 이제 씬 시드 길이 × `sceneLengthMultiplier`(기본 12)로 예산을 도출하고 2,000~20,000자로 제한하며, `sceneLengthMultiplier: 0`으로 이 동작을 끌 수 있습니다. 같은 예산을 생성과 재작성이 공유합니다.
- 같은 문장·후렴구가 반복되어도 걸러지지 않던 문제를 고쳤습니다. 기존 반복 제한이 이미지 표현에만 적용되던 것을 동일 문장과 후렴까지 확장했습니다.
- grounding 제안이 인물 카드 이름 대신 카드 ID(슬러그)를 그대로 노출하던 문제를 고쳤습니다.

## [0.6.1] - 2026-08-21

### 제거

- seedcoat `.seed` 저장소 아카이브 기능을 제거했습니다. `Storyboard: Create/Sync/Export Project ... Seed` 명령 3개와 탐색기 `.seed` 메뉴, `@seedcoat/wasm` 의존성이 사라졌습니다. 작품은 git 워크스페이스 자체로 관리하며, 별도 교환용 아카이브 포맷을 두지 않습니다.
- 스캐폴딩 잔재였던 `Storyboard: Hello World` 명령을 제거했습니다.

### 수정

- 모노레포 재구조화 이전 경로를 import해 기동이 불가능했던 씬 커버리지 진단 하네스(`coverageCheck.harness.ts`)를 수리했습니다.
- 웹뷰 뷰에는 나타날 수 없어 동작하지 않던 카드 ID 변경 트리 컨텍스트 메뉴 기여 2건을 정리했습니다. 탐색기 우클릭·명령 팔레트 경로는 그대로 동작합니다.
- `Storyboard: Promote Card Candidates` 명령 제목의 한국어 번역 누락을 보완했습니다.

### 구조

- 참조되지 않는 코드와 의존성을 정리했습니다: 미사용 모듈·함수·타입 삭제, 내부 전용 심볼의 export 해제, 봇의 `js-yaml` 등 미사용 의존성 제거. 사용자 기능 변화는 없습니다.
- `ARCHITECTURE.md` 등 문서가 가리키던 재구조화 이전 파일 경로 13곳을 실제 위치로 정정했습니다.

## [0.6.0] - 2026-08-01

### 추가

- 텔레그램 동반 봇 **storygram**을 추가했습니다. 확장이 여는 것과 **같은** 워크스페이스를 편집하는 상주 봇으로, 이동 중에도 초안을 읽고(`/read`) 씬 시드를 만들거나 고치고(`/scene`) 카드를 편집하며, 초안 생성을 큐에 넣고 진행 상황을 받아볼 수 있습니다. 저장에 성공한 파일은 항상 커밋되고, 쓰기 직전에 대상 파일을 다시 읽어 확장과 동시에 편집해도 서로의 변경을 덮어쓰지 않습니다.
- `Storyboard: 텔레그램 봇 설정…` 온보딩 마법사를 추가했습니다. 토큰을 텔레그램에 먼저 검증한 뒤 허용 채팅 ID·워크스페이스·기본 프로바이더를 받아 `~/.storygram/config.json`을 0600 권한으로 저장합니다. 아직 초기화하지 않은 폴더를 고르면 그 자리에서 Init을 실행해 마법사를 이어가고, macOS에서는 의존성 설치 → 빌드 → launchd 등록까지 진행률과 함께 수행한 뒤 봇이 실제로 응답하는지 확인하고 대시보드를 엽니다. 토큰은 로그와 오류 메시지에 남지 않습니다.
- 상태바에 봇 상태(미설정·관찰 불가·중지·실행 중)를 표시하고, `Storyboard: 텔레그램 봇 대시보드 열기`로 루프백 운영 패널을 엽니다. 미설정 상태를 클릭하면 설정 마법사로 이어집니다.
- 설정을 마친 뒤에도 봇 설정을 고칠 수 있는 경로를 추가했습니다. 설정 패널의 **텔레그램 봇** 탭에서 허용 채팅 ID와 기본 프로바이더를 편집하고, «이 작품을 봇에 연결»로 봇이 바라보는 워크스페이스를 바꿉니다. `Storyboard: 텔레그램 봇 설정 파일 열기`는 JSON 스키마 검증(자동완성·오타 검사)이 붙은 편집기를 열고, `Storyboard: 텔레그램 봇 재시작`은 launchd로 재시작한 뒤 헬스를 확인합니다. 토큰은 마스킹된 힌트만 보이며 변경은 마법사에서만 가능합니다.

### 수정

- 카드의 `role: support`가 조연(supporting)이 아니라 단역(extra)으로 조용히 강등되던 문제를 고쳤습니다. 정사 주입과 생성 파이프라인의 인물 취급에 영향을 주던 문제입니다.
- 스키마가 모르는 role 값(`주연`, `Main` 등)이 조용히 단역으로 강등된 뒤 재저장 시 원래 값이 사라지던 문제를 고쳤습니다. 이제 대소문자와 알려진 별칭만 보정하고, 그래도 모르는 값이면 Problems 패널에 어느 필드가 왜 거부됐는지 함께 표시합니다.
- 시놉시스 값 안에 마크다운 마커(`## `, `- `, `_미작성_`)나 개행이 들어가면 저장·재읽기 과정에서 내용이 잘리거나 항목이 분해되던 문제를 고쳤습니다.
- 시놉시스 필드 하나가 불량이면 시놉시스 전체가 빈 값으로 초기화되던 문제를 고쳤습니다. 이제 유효한 필드는 보존하고 불량 필드만 기본값으로 대체합니다.
- 경로 판정이 대소문자를 무시해 `SCENE/01.TXT` 같은 파일이 씬으로 분류된 뒤 파서에서 거부되던 불일치를 고쳤습니다.

### 구조

- 저장소를 npm workspaces 모노레포로 재배치했습니다. 확장은 `apps/vscode`으로 이동했고, 워크스페이스 파일 포맷(`@storyboard/story-format`), AI 엔진(`@storyboard/story-ai`), 씬 생성 파이프라인(`@storyboard/story-pipeline`), git 동기화 계층(`@storyboard/story-git`)을 공유 패키지로 분리했습니다. 확장의 사용자 동작과 VSIX 내용물은 변하지 않았습니다.
- 봇은 `apps/bot`에 있으며 확장과 동일한 코덱·파이프라인을 소비합니다. 따라서 어느 쪽에서 저장해도 같은 바이트가 기록됩니다.

## [0.5.2] - 2026-07-24

### 추가

- 수동 **이야기 완결**과 **씬 기반 카드 구성** 기능을 추가했습니다. 두 작업은 Command Palette, Studio, 해당 사이드바 뷰 헤더에서 같은 유스케이스로 실행하며, AI 제안 → 선택 → VS Code native diff → 최종 확인 → 적용 순서로 동작합니다.
- 완결은 기존 `scene/*.txt`를 수정하지 않고 마지막 번호 뒤에만 새 씬을 추가합니다. 카드 구성은 씬만 새 사실 근거로 사용해 신규 카드와 기존 카드 보강안을 함께 제안하고, 별칭·태그도 선택 반영할 수 있습니다.
- Studio 대화를 세션 단위로 `.storyboard/cache/studio-sessions/`에 저장합니다. 뷰를 다시 열면 최신 세션이 복원되고, 최근 세션 목록(20개 유지)에서 지난 대화를 다시 열거나 새 대화를 시작할 수 있습니다.
- Studio 입력창에 슬래시 명령을 추가했습니다. `/`를 입력하면 현재 대상에 맞는 명령이 자동완성되고, 선택한 슬래시 명령은 승인 단계 없이 바로 실행됩니다.
- Studio에 **초안 압축** 기능을 추가했습니다. 설정 가능한 분량 한도에 맞춰 초안을 줄이고, VS Code diff 검토 후에만 적용합니다. 자동 리비전 출력이 비정상이거나 너무 짧으면 기존 초안을 보존하고, 과도하게 쪼개진 상황(situation)은 병합해 반복 장면 생성을 줄입니다.
- 스키마를 위반한 `character/*.card`·`background/*.card` 파일을 Problems 패널에 필드 단위 오류 위치와 함께 표시합니다. 기존에는 위반 카드가 씬 생성에서 무경고로 제외됐습니다.
- 씬 frontmatter에 선택적 `targetWordCount`(양의 정수)를 추가했습니다. 지정하면 장르 포맷 단계가 씬별 목표 분량 지시를 받습니다.
- codex reasoning effort 설정 `storyboard.providers.codex.reasoningEffort`를 추가했습니다. 빈 값이면 CLI 기본값을 사용합니다.
- 모델 카탈로그에 GPT-5.6 Codex 모델(Sol·Terra·Luna)을 추가하고, codex 기본 모델을 Sol로 지정했습니다.
- 헤드리스 하네스를 보강했습니다: 실행 종료 시 태스크별 토큰 사용량 요약 출력, 씬별 배경 카드 부착 결과 로그와 미부착 경고, `SCENE_CLI_TIMEOUT` 환경변수로 CLI 타임아웃 오버라이드(기본 600초), `SCENE_EFFORT` 환경변수로 codex reasoning effort 지정.

### 수정

- codex CLI 출력을 청크마다 디코딩해 멀티바이트 문자가 청크 경계에서 U+FFFD 대체문자로 손상되던 문제를 고쳤습니다. 이제 프로세스 종료 시 한 번만 UTF-8 디코딩합니다.
- 초안 검수·재작성 단계가 캐릭터 카드를 전달받지 못해 인물 말투가 프로젝트 품질 기준에 밀려 바뀌던 문제를 고쳤습니다. 두 단계 모두 장면 캐릭터 카드를 받고 카드 보이스를 최우선으로 적용합니다.
- Studio 웹뷰가 project 대상을 인식하지 못하고 none으로 강등하던 문제를 고쳤습니다.
- 수동 초안 압축 결과가 목표 분량보다 짧아도 유효하면 거부하지 않고 diff 검토로 전달하도록 고쳤습니다.
- 아키텍처 재구성 이후 낡은 임포트 경로로 실패하던 헤드리스 하네스 실행을 고쳤습니다.

### 안전

- 제안 입력과 대상 파일의 SHA-256 스냅샷을 적용 직전에 검증합니다. 검토 중 변경되면 자동 재배치·덮어쓰기 없이 다시 제안하도록 중단합니다. 적용은 단일 `WorkspaceEdit`를 사용하지만 크래시까지 보장하는 완전 트랜잭션은 아닙니다.

## [0.5.1] - 2026-07-19

### 변경

- 확장 기능의 동작을 유지하면서 extension host 코드를 bootstrap·application·domain·infrastructure·presentation 계층으로 재구성했습니다. 의존성 방향을 검증하는 아키텍처 검사와 유스케이스 단위 테스트를 추가해 이후 변경의 안정성을 높였습니다.

## [0.5.0] - 2026-07-04

### 추가

- 항상 보이는 **Storyboard · Studio** 사이드바 패널을 추가했습니다. 현재 활성 `draft/*.md`·`scene/*.txt`를 자동으로 인식해, 초안 재생성·문법/연속성 검사·카드 기반 보충·선택 영역 확장/업데이트·지시문 편집(Edit Selection 입력란)과 씬 초안 생성·포맷 적용을 한곳에서 실행합니다. 본문을 스크롤해도 버튼이 사라지지 않으며, 패널 헤더를 우측 Secondary Side Bar로 옮길 수 있습니다.
- 초안 생성 시 장면 전환마다 구분자를 삽입하는 기능을 추가했습니다. `storyboard.draft.sceneBreakEnabled`로 켜고 `storyboard.draft.sceneBreakSeparator`로 `---` 구분선 또는 줄바꿈 반복 횟수(1~10)를 선택할 수 있으며 기본값은 꺼짐입니다. 켜면 장르 포맷 AI 호출이 장면 단위로 실행되어 호출 수가 늘어나고, 설정을 바꾸면 다음 Generate Draft에서 캐시 없이 재생성됩니다.

### 변경

- 스크롤 시 사라지던 초안/씬 상단 CodeLens 버튼을 제거하고, 동일한 동작을 Studio 패널로 옮겼습니다. 같은 동작은 명령 팔레트로도 계속 실행할 수 있습니다.

## [0.4.4] - 2026-06-30

### 추가

- 재생성 없이 현재 카드·정전(canon)을 기존 초안에 반영해 보충하는 기능을 추가했습니다. 초안 CodeLens의 `✨ Augment from Cards`는 본문 전체를, `🪄 Update Selection`은 선택한 영역만 보충하며, 사건 전개·문체·직접 편집분을 보존합니다. 적용 전 VSCode 네이티브 diff로 변경 전/후를 확인하고 **적용**을 눌러야 반영되며, `storyboard.draft.keepHistory`가 켜져 있으면 직전 초안을 `.draft` 히스토리에 보관합니다.
- 선택 영역을 지시문으로 편집하는 기능을 추가했습니다. 초안 CodeLens의 `✏️ Edit Selection...`을 클릭하면 수정 방향 입력 창이 열립니다. 지시문(예: "더 긴장감 있게", "짧게 줄여서")과 현재 카드·정전 정보를 함께 AI에 전달해 선택 영역을 재작성하고, 동일한 diff 미리보기·적용 흐름을 거칩니다.
- 초안 인라인 완성이 연결된 씬 파일의 의도, 등장 캐릭터, 배경 정보를 함께 참고하도록 보강했습니다. 캐릭터 말투와 배경 묘사를 짧게 포함해 현재 장면에 맞는 이어쓰기 제안을 만들며, 동일 장면 컨텍스트는 짧게 캐시해 키 입력 중 반복 I/O를 줄입니다.

### 수정

- 초안 보강 diff 미리보기에서 실제 초안 파일을 before 쪽으로 직접 열어 VSCode 자동 저장이나 CodeLens 동작이 간섭할 수 있던 문제를 고쳤습니다. 이제 변경 전/후 모두 읽기 전용 가상 문서로 보여주고, 사용자가 **적용**을 누를 때만 실제 초안을 갱신합니다.

### 테스트

- 초안 보강 프롬프트, 지시문 기반 선택 편집, AI 서비스 보강 호출, 인라인 완성 씬 컨텍스트 포맷 테스트를 추가했습니다.

## [0.4.3] - 2026-06-28

### 추가

- Characters·Backgrounds 사이드바 헤더에 **추천**(✨) 버튼을 추가했습니다. 클릭 시 프로젝트 전체 `scene/*.txt`와 `draft/*.md`를 LLM으로 분석해 본문에 등장하지만 아직 카드가 없는 인물·배경을 QuickPick으로 제안하고, 선택한 항목만 최소 카드 파일로 생성합니다. 이미 카드에 등록된 이름과 별칭은 제안에서 제외됩니다.

### 수정

- 한국어처럼 ASCII 문자가 없는 이름의 카드를 추천으로 생성할 때 `background-2`, `background-3`처럼 의미 없는 파일명이 붙던 문제를 고쳤습니다. 이제 파일 ID 지정 창이 떠서 직접 이름을 입력할 수 있습니다.

## [0.4.2] - 2026-06-28

### 추가

- 카드 에디터에 **수집** 탭을 추가했습니다. `character/*.card`와 `background/*.card`에서 해당 카드가 등장하는 `draft/*.md`를 스캔해 LLM으로 카드 보강 제안을 만들고, 사용자가 선택한 항목만 카드 YAML에 반영할 수 있습니다.
- 카드 수집 제안 타입과 적용 로직을 추가했습니다. 캐릭터는 속성·관계·아크·특성·최근 대사·설명·화법·욕망을, 배경은 설명·감각·시간·날씨·등장인물을 제안합니다.
- 카드 수집용 AI 빌더와 `backgroundFactExtraction` 작업을 추가했습니다. 씬별 제안을 모아 중복을 제거하고, 기존 값이 있는 항목은 추가가 아니라 수정 제안으로 표시합니다.
- `cards.collect`, `cards.applyCollect`, `cards.previewCollect` RPC 메시지를 추가하고 카드 커스텀 에디터 provider에 연결했습니다.
- 수락할 제안을 고른 뒤 VSCode 네이티브 diff 편집기로 현재 카드와 반영 예정 YAML을 비교하는 미리보기 흐름을 추가했습니다.

### 수정

- 수집 diff 미리보기 가상 문서가 `.card` 확장자 때문에 다시 카드 커스텀 에디터로 라우팅되어 열리지 않던 문제를 고쳤습니다.
- 별칭으로만 언급된 캐릭터도 카드 수집 대상과 추출 컨텍스트에 포함되도록 고쳤습니다. 예를 들어 본명이 아닌 호칭으로 등장한 대사와 서술도 수집 후보에 반영됩니다.

### 문서

- README와 작가 가이드에 카드 에디터 수집 탭, 항목 검토, diff 미리보기, 선택 반영 흐름을 문서화했습니다.

### 테스트

- 카드 수집 제안 타입, 신규/수정 필터링, 적용 로직, 배경 정보 추출, 캐릭터 설명·화법·욕망 수집, 별칭 기반 수집, 메시징 레지스트리 테스트를 추가·갱신했습니다.

## [0.4.1] - 2026-06-27

### 추가

- 초안 히스토리 옵션 `storyboard.draft.keepHistory`(기본 꺼짐)를 추가했습니다. 켜면 Generate/Regenerate가 `draft/<scene>.md`를 덮어쓰기 직전에 이전 초안을 `.draft/<scene>/<yyyy-mm-dd-hh-mm>-rev-NN.md`로 보관합니다. `.draft/`는 `draft/`처럼 `.gitignore`로 제외됩니다.
- 초안 생성 결과에서 캐릭터별 속성·관계·아크 후보를 추출해 `.storyboard/cache/cards/`에 저장하는 카드 후보 흐름을 추가했습니다. `storyboard.draft.updateCardsAfterGenerate`가 켜져 있을 때 동작하며, `Storyboard: Promote Card Candidates` 명령으로 사용자가 선택한 후보만 캐릭터 카드에 반영합니다.
- 카드 후보 검증 옵션 `storyboard.draft.verifyCardCandidates`(기본 켜짐)를 추가했습니다. 추출된 후보를 초안 본문과 다시 대조해 명시적으로 확인되는 후보만 승격 대기 목록에 남깁니다.
- 캐릭터 카드에 `desire` 항목 목록을, 배경 카드에 `aliases`·`time`·`weather`·`senses` 필드를 추가했습니다. 페르소나 생성과 배경 묘사 프롬프트가 새 필드를 컨텍스트로 사용합니다.
- 씬 frontmatter에 `location`이 없어도 본문에서 배경 이름·별칭을 감지해 씬 컨텍스트에 배경 카드를 연결합니다.
- 초안 생성 후 감지된 캐릭터를 해당 배경 카드의 `characterIds`에 자동 추가하는 흐름을 추가했습니다.

### 변경

- 캐릭터 `voice`·`description`과 배경 `description`을 긴 산문 대신 **항목 목록(`string[]`)**으로 입력하도록 바꿨습니다. 카드 편집기에서 항목 추가/삭제로 관리하며, YAML에는 시퀀스로 저장됩니다.
- 산문으로 작성된 기존 카드를 줄 단위 목록으로 변환하는 `Storyboard: 카드 텍스트 필드를 목록형으로 변환`(`storyboard.cards.migrateTextToList`) 명령을 추가했습니다.
- 카드 편집기에서 수동 입력 필드와 AI가 관리하는 후보·관계·아크 필드를 분리했습니다. 관계·아크·속성은 초안에서 후보로 모은 뒤 승격 명령으로 반영하는 흐름에 맞췄습니다.
- `storyboard.draft.updateCardsAfterGenerate` 설명과 동작 범위를 확장했습니다. 기존 traits/recent dialogue 업데이트에 더해 배경 캐릭터 연결과 카드 후보 추출까지 포함합니다.
- 캐릭터 `attributes`를 페르소나 캐시와 페르소나 생성 프롬프트에 반영하도록 바꿨습니다.
- Guerrila harness의 재생성 흐름도 기존 초안을 `.draft/` 히스토리에 보관하도록 맞췄습니다.
- `npm run lint`가 extension host `tsconfig.json` 타입체크까지 실행하도록 보강했습니다.

### 수정

- 캐릭터 `voice`·`description`·`attributes`와 배경 `description`·`senses`가 바뀌면 씬 캐시가 올바르게 무효화되도록 고쳤습니다.
- 승격된 카드 후보는 후보 캐시에서 제거하고, 모두 반영된 후보 파일은 삭제하도록 고쳤습니다.
- 로컬 개발용 `TODO.md`가 Git과 VSIX 패키지에 포함되지 않도록 제외했습니다.

### 문서

- README, 아키텍처 문서, 수동 QA 문서, 카드 파라미터 영향 리포트에 카드 후보 승격, 초안 히스토리, 목록형 카드 필드, 배경 감지와 새 카드 필드 흐름을 반영했습니다.
- 카드/씬 포맷 스킬 문서에 목록형 카드 텍스트 필드와 초안 히스토리 규칙을 반영했습니다.

### 테스트

- 카드 후보 추출·검증·승격·캐시 정리, 배경 캐릭터 자동 업데이트, 카드 텍스트 목록 변환, 초안 히스토리, 배경 자동 감지, 카드 스키마와 씬 캐시 무효화 테스트를 추가했습니다.
- 카드 편집기 수동 필드, YAML 편집, Seed 가져오기/리맵, 페르소나·배경 묘사 프롬프트 관련 회귀 테스트를 갱신했습니다.

## [0.4.0] - 2026-06-25

### 추가

- 캐릭터별 페르소나와 배경별 분위기 묘사를 `.storyboard/cache/personas/`와 `.storyboard/cache/backgrounds/`에 저장하는 카드 단위 메모리를 추가했습니다. 카드 내용이 바뀌면 `cardHash`로 캐시를 무효화합니다.
- `backgroundDescription` AI 작업을 추가했습니다. 배경 카드에서 장소·시대 분위기와 감각 묘사를 생성하고, 씬 대화 생성 컨텍스트에 주입합니다.
- 검수 이슈를 `canon` / `persona` / `narrator` 에이전트로 라우팅하는 결정적 매핑을 추가했습니다. 보이스 이슈는 가능하면 캐릭터 name·alias로 카드 범위를 좁히고, 전역 이슈만 기존 전체 재작성으로 처리합니다.
- `/scene-quality` 워크플로와 장면 품질 루브릭 문서를 추가했습니다. 커버리지 게이트, 서사력·연결성·장면 구분·전환 축, 6차원 품질 점수를 같은 기준으로 기록합니다.

### 변경

- `storyboard.draft.reviseAfterGenerate` 기본값을 `true`로 바꿨습니다. Generate / Regenerate / Generate All 직후 검수·재작성 루프가 자동으로 이어져 한 동작으로 검수된 초안을 생성합니다.
- Claude Code와 Codex CLI provider에 `timeoutMs` 설정을 추가하고 기본 생성 타임아웃을 10분으로 늘렸습니다.
- 씬 생성 파이프라인, 장편 생성 파이프라인, 초안 생성 명령, AI provider 구성, 설정 화면, 메시징 계약을 작은 단계와 모듈로 분리했습니다. 사용자 동작은 유지하면서 테스트 가능한 경계를 넓혔습니다.
- Codex JSONL 파싱, 페르소나 라인 구성, AI 응답 보정, revise-after-generate 게이트, Seed 파일 I/O 흐름의 중복을 줄였습니다.

### 문서

- 아키텍처 문서에 다중 에이전트 협업 모델, 카드 단위 메모리, 검수 피드백 라우팅, 드로잉 에이전트 배경 묘사 흐름을 반영했습니다.
- 작가 가이드에 초안 생성 후 자동 검수·재작성 기본 동작을 반영했습니다.
- 카드 파라미터가 draft 생성 품질에 실제로 미치는 영향을 정리한 리포트를 추가했습니다.
- Claude/Cursor 규칙과 스킬의 canonical 위치를 `.claude/` 중심으로 정리하고, Cursor 미러를 동기화했습니다.

### 테스트

- 카드 메모리 직렬화·검증, 페르소나 캐시 재사용·무효화, 배경 묘사 캐시, 검수 라우팅, 설정 스냅샷, CLI 타임아웃, 메시징 레지스트리 분리 테스트를 추가했습니다.
- 장면 생성 파이프라인과 revise workflow 테스트를 보강했습니다.

## [0.3.3] - 2026-06-24

### 추가

- 캐릭터 카드에 `aliases`와 `voice` 필드를 추가했습니다. 씬 본문 캐릭터 감지와 상황별 페르소나 스코핑이 이름뿐 아니라 별칭도 인식합니다.
- 씬 frontmatter의 `relationStage`와 작품 설정의 시점·장르·문체 제약을 묶는 style directive를 추가했습니다. 페르소나 생성, 대사 생성, 장르 포맷, 초안 비평 프롬프트가 이를 반영합니다.
- `sceneCoverage` AI 작업과 `StoryboardAIService.checkSceneCoverage`를 추가했습니다. 입력 비트가 초안에 빠졌거나 순서가 바뀐 경우를 JSON으로 검사하고 요약할 수 있습니다.
- 실제 CLI provider로 장편 씬 생성과 장면 커버리지 검사를 돌릴 수 있는 별도 Vitest harness 설정과 스크립트를 추가했습니다.

### 변경

- 씬 생성 파이프라인이 각 상황의 실제 참여 인물 페르소나만 넘기고, 이전 상황 원문 대신 앞서 생성된 대사 tail을 다음 상황 컨텍스트로 이어 씁니다.
- 상황 추출·대사 생성·장르 포맷 프롬프트를 보강해 비트를 빠뜨리지 않고, 전환·행동·내면·갈등을 장면으로 극화하도록 조정했습니다.
- 긴 씬의 최종 포맷을 글자 예산 단위로 나누어 처리합니다. 모델이 분량 한계나 선택지 안내 같은 메타 응답을 반환하면 원고에 섞지 않고 원본 대사 조각을 유지합니다.
- 초안 생성 후 캐릭터 카드 자동 업데이트는 `storyboard.draft.updateCardsAfterGenerate` 설정을 켰을 때만 실행되도록 기본값을 껐습니다.

### 문서

- 아키텍처 예시에 캐릭터 `voice`와 씬 `relationStage` frontmatter를 반영했습니다.

### 테스트

- 별칭 기반 캐릭터 감지, style directive, scene coverage 파싱·요약, 프롬프트 지시문, 씬 생성 파이프라인의 페르소나 스코핑·컨텍스트 연결·청크 포맷 테스트를 추가했습니다.

## [0.3.2] - 2026-06-22

### 추가

- 스토리 바이블 정전 사실에 `validFrom` / `validUntil` 유효 범위와 `keywords` 활성화 조건을 추가했습니다. 씬 순번에 맞는 canon 버전만 주입하고, 씬 본문에 키워드가 등장하면 등장 엔티티가 아니어도 관련 사실을 추가로 주입합니다.
- `Storyboard: Slop Check (Draft)` 명령과 선택적 `storyboard.slop.realtimeEnabled` 설정을 추가했습니다. 초안에서 상투 표현, “단순히 X가 아니라 Y”류 대조 구문, 과도하게 반복되는 3어절 표현을 진단합니다.
- `storyboard.draft.reviseAfterGenerate` 설정을 추가했습니다. 켜면 `Generate Draft` / `Generate All Drafts`로 새 초안을 생성한 직후 기존 연속성·비평 검수 재작성 게이트를 실행합니다.
- `storyboard.draft.reviseScoreThreshold` 설정과 비평 루브릭 점수를 추가했습니다. 기준 점수 이상이고 high 연속성 이슈가 없으면 검수·재작성 루프를 조기 통과할 수 있으며, 최종 검사 보고서에도 `비평 점수: NN/100`을 표시합니다.
- `Storyboard: Canon Diff Report`가 같은 subject/key의 시간 범위별 canon 버전을 “설정 변경 타임라인”으로 함께 표시합니다.

### 변경

- 씬 생성 컨텍스트가 2번째 이상 씬에서 `manuscript/SUMMARY.md`의 롤링 요약(최대 2000자)을 이전 장면 컨텍스트로 우선 사용합니다. 요약 파일이 없거나 비어 있으면 기존처럼 이전 초안 tail 1000자로 fallback합니다.
- 연속성 검사 결과에 `high` / `low` 심각도를 도입했습니다. `high`만 재작성 차단 이슈로 세고, 실시간 진단에서는 `high`를 Warning, `low`를 Information으로 표시합니다.
- 자동 승격되는 바이블 후보는 `sourceScene`을 해석할 수 있으면 해당 씬을 `validFrom`으로 삼고, 같은 subject/key의 이후 버전과 구분되는 id를 부여합니다.
- Codex CLI provider의 기본 모델을 `gpt-5.5`로 바꾸고, 기존 `gpt-5-codex` 설정은 현재 catalog 기본값으로 보정합니다.
- Codex CLI는 ChatGPT 구독 기반 사용을 반영해 토큰 사용량만 기록하고 USD 비용은 `0`으로 기록합니다. 이전 gpt-5-codex API 요금 환산 추정치는 제거했습니다.

### 수정

- Codex CLI가 JSONL 오류 이벤트와 함께 실패할 때, 일반 종료 코드 메시지 대신 CLI가 제공한 구체적인 실패 사유를 표시합니다.

### 문서

- README, 아키텍처 문서, 작가 가이드, 수동 QA 문서에 canon 키워드 주입, 롤링 요약 컨텍스트, 검수 점수 기준, Codex 비용 기록 변경을 반영했습니다.

## [0.3.1] - 2026-06-22

### 추가

- Claude Code(`claude`)·Codex(`codex`) CLI provider를 추가했습니다. API 키 없이 각 CLI의 자체 로그인(구독)으로 생성하며, 실행 명령·모델은 설정과 **연결** 탭에서 바꿀 수 있습니다.
- 연결 테스트가 CLI 바이너리를 찾지 못한 경우(`ENOENT`)를 그 밖의 실패와 구분해 «CLI 미설치»로 표시합니다.
- Claude Code 비용은 CLI가 보고하는 `total_cost_usd`를 기록하고, Codex 사용량은 `codex exec --json`에서 파싱합니다. Codex 비용은 gpt-5-codex API 요금으로 환산한 추정치이며 실제 청구액이 아닙니다.

### 변경

- CLI provider는 실시간 토큰 스트리밍 대신 전체 결과를 한 번에 전달하고, 인라인 완성은 CLI provider에서 비활성화합니다.
- CLI provider에서는 각 CLI가 노출하지 않는 `temperature`와 출력 토큰 상한(`maxTokens`)을 적용하지 않습니다.

### 수정

- Claude Code와 Codex 연결 테스트가 CLI 실행 가능 여부뿐 아니라 로그인 상태까지 확인하도록 보강했습니다.
- 사용자 지정 CLI command가 stdin 소비 전 종료해도 `EPIPE`로 확장이 중단되지 않도록 보호했습니다.
- Codex `--json` 이벤트 파싱을 강화해 빈 usage 이벤트가 앞선 사용량을 0으로 덮어쓰지 않도록 했습니다.

### 문서

- README와 아키텍처 문서에 CLI provider 지원 범위, 인증 방식, 비용 추정, 스트리밍·인라인 완성 제한을 정리했습니다.

## [0.3.0] - 2026-06-21

### 추가

- `Storyboard: Generate Novel` 명령으로 작품 설정 → outline → 시드 → 장별 초안·검수 → 조립 → 검사 → 요약까지 원클릭 실행하고 단계별로 재개할 수 있습니다.
- `Storyboard: Canon Diff Report`(`storyboard.bible.canonDiff`)가 아직 승격되지 않은 설정 후보를 `canon.yaml`과 대조해 `manuscript/CANON.md`로 정리합니다.
- `Storyboard: Export Draft…`(`storyboard.draft.export`)가 조립 원고를 Markdown 또는 일반 텍스트로 내보냅니다. (PDF/DOCX는 후속)
- 검수·재작성 결과와 지시를 `.storyboard/outline/revision-plan.yaml`에 scene 단위로 누적합니다.
- 작품 계약에 문체 제약(`styleConstraints`)·품질 기준(`qualityCriteria`)을 추가하고 **작품 계약** 설정 탭에서 편집합니다. 해당 기준은 draft critique 프롬프트에도 반영됩니다.
- `chapters.yaml`에 chapter/scene 단위 목표 분량(`targetWordCount`)을 저장합니다.
- outline 기반 씬 시드에 갈등(`conflict`), 반전(`twist`), 필요 설정(`neededCanon`), 목표 분량을 포함합니다.
- 확장 UI 다국어(i18n) 메커니즘(`package.nls.json` / `package.nls.ko.json`)을 도입하고 모든 명령 제목을 외부화했습니다.

### 변경

- `chapters.yaml`가 씬보다 최신이면 Scenes 사이드바에 "outline" stale 배지를 표시합니다.

## [0.2.4] - 2026-06-18

### 추가

- 스토리 바이블 도메인과 파일 I/O를 추가했습니다. 작품 설정·캐논을 워크스페이스에 저장·로드할 수 있습니다.
- 씬 생성 시 스토리 바이블 캐논을 프롬프트에 주입합니다.
- 연속성 검사(continuity check) AI 작업을 추가했습니다.
- 초안에 연속성 진단 결과를 표시하고, 초안 CodeLens에서 연속성 검사를 실행할 수 있습니다.
- 바이블 후보(bible candidate) 팩트 저장소와 설정 팩트 추출 AI 작업을 추가했습니다.
- 초안 생성 후 바이블 후보를 자동 추출하고, 후보를 캐논으로 승격하는 명령을 추가했습니다.
- 배경 카드 폼에서 관련 캐릭터(`characterIds`)를 편집할 수 있습니다.

### 문서

- 스토리 바이블·연속성 검사·바이블 후보 승격 흐름을 문서화했습니다.
- 작가용 시작 가이드(`GUIDE.md`)를 추가했습니다.
- 제품 계획을 “작가 보조형 픽션 IDE”에서 **원클릭 장편 생성 IDE / Autonomous Fiction Studio** 방향으로 재정렬했습니다. `ARCHITECTURE.md`, README, agent rule 요약을 같은 용어로 갱신했습니다.

## [0.2.3] - 2026-06-14

### 변경

- `.seed` 가져오기/보내기를 `@seedcoat/wasm` v0.4.0 저장소 엔진 API(`load`/`checkoutSnapshot`, `init`/`note`/`save`)로 전환했습니다. `.seed`는 이제 전체 변경 이력을 보존하는 비암호화 포터블 아카이브(`seedcoat archive v1`)입니다.
- 가져오기/보내기에서 패스프레이즈 입력과 암호화/복호화 진행 알림을 제거했습니다. 내보내기 시 비암호화 고지를 1회 표시합니다.
- Seed 오류 메시지를 신규 오류 코드 체계(`UNSUPPORTED_FORMAT`, `HASH_MISMATCH` 등 11종)에 맞게 교체했습니다.
- `@seedcoat/wasm`이 순수 TypeScript 패키지가 되어 확장 번들에 직접 포함합니다. `out/vendor` 복사 단계(`scripts/copy-seedcoat.mjs`)와 동적 import 로더를 제거했습니다.

### 제거

- 구 암호화 `.seed`(seedcoat v0.2) 지원을 제거했습니다. 해당 파일은 지원하지 않는 형식으로 거부되며, 보낸 쪽에서 v0.4 형식으로 다시 내보내야 합니다.

## [0.2.2] - 2026-05-28

### 추가

- 캐릭터 카드에 역할(`protagonist` / `supporting` / `extra`)을 명시할 수 있는 스키마와 에디터 입력 UI를 추가했습니다.
- 캐릭터 사이드바를 역할별 그룹(주연·조연·엑스트라·미분류)으로 나누고, 그룹별 접기/펼치기와 카드 수 표시를 지원하도록 개선했습니다.
- 카드 에디터·미리보기의 `StoryboardCard`에 카드 게임 스타일 프레임(캐릭터/배경 톤 구분)과 역할 배지(주연·조연·엑스트라 색·문양)를 적용했습니다.
- 카드 에디터에 원본 YAML 편집 패널을 추가했습니다. 저장 전 YAML 검증 오류를 표시하고, 성공 시 카드 데이터를 다시 동기화합니다.
- 캐릭터 관계 미리보기 패널을 추가해 캐릭터의 arc 흐름과 관계 목록을 카드 에디터에서 바로 확인할 수 있습니다.
- 캐릭터 관계 표시에서 관계 대상 ID를 캐릭터 이름/역할 정보와 함께 보여주는 roster lookup을 추가했습니다.

### 변경

- 카드 에디터 Overview 필드(List/Arc/Relations/Key-Value)의 항목 추가·삭제 버튼을 `+` / `-` 아이콘 `IconButton`으로 교체했습니다.
- 카드 미리보기 영역을 `PreviewPanel`로 분리해 카드 에디터 본문과 미리보기 렌더링 책임을 나눴습니다.
- Arc와 Relations 입력 UI의 배열 편집 흐름을 정리하고, 관계 미리보기와 동일한 데이터 형태를 사용하도록 맞췄습니다.
- 초기 프로젝트 템플릿과 경로 규칙에서 더 이상 쓰지 않는 character relation 샘플 경로 생성을 제거했습니다.

## [0.2.1] - 2026-05-24

### 수정

- 탐색기 F2 rename 시 카드 본문 `id` 갱신이 적용되지 않던 문제를 수정했습니다. `onWillRenameFiles` 시점에 `oldUri`에 텍스트 edit를 걸도록 변경했습니다.
- 사이드바·명령 팔레트 **ID 변경**이 `fs.rename`만 호출해 참조·프로필이 갱신되지 않던 문제를 수정했습니다. `workspace.applyEdit` + `renameFile`로 participant 경로와 통일했습니다.

### 추가

- 탐색기에서 `*.card` 파일 rename 시 본문 `id`, 다른 카드 참조, 캐릭터 프로필 이미지를 함께 갱신합니다. 사이드바 컨텍스트 메뉴의 **ID 변경** 명령(`storyboard.character.rename`, `storyboard.background.rename`)도 같은 경로를 사용합니다.
- Seed 가져오기·동기화 직후 **ID 매핑 검토** QuickPick을 추가했습니다. 카드별 새 ID를 지정할 수 있으며, **변경 없이 계속**을 선택하면 기존 흐름과 동일합니다.

## [0.2.0] - 2026-05-22

### 변경

- `doc/` 트리를 비추적 `.doc/`으로 옮기고(원격 저장소에는 포함하지 않음), 공개 문서는 루트 `ARCHITECTURE.md`, `STORYBOARD_ALIGNMENT.md`, `EXTENSION_QA.md`, `RELEASE.md`, `GUIDE.md`로 정리했습니다. README, AGENTS, Cursor/Clinerules, 스킬 문서의 링크를 갱신했습니다.
- `.seed` 파일을 평문 JSON envelope(`version: "2.0.0"`)에서 **`@seedcoat/wasm` v0.2.0 암호화 컨테이너**로 완전히 이관했습니다(하드 컷오버). 이전 평문/구버전 `.seed`는 더 이상 지원하지 않으며 `LEGACY_FORMAT_REJECTED`로 거부됩니다.
- 배경 카드 스키마를 단일 `type: background`에서 **판별 유니온(`location` / `temporal` / `social`)**으로 교체하고, 공통 필드(`characterIds`, `tags`)와 `locationKind`(location 전용)를 도입했습니다. 이전 `concept` / `country` / `category` 필드는 제거되었습니다.
- 프로젝트 메타데이터의 `settings`를 `editor`(`scenePrefixDigits`, `trackDraft?`)와 작품 단위 `setting`(genre/country/concept/tags/description)으로 분리했습니다.
- 루트 `SEED-FORMAT.md`를 제거하고, 컨테이너 명세는 [seedcoat](https://github.com/webfic/seedcoat)를 단일 진실원으로 둡니다. Storyboard 정책은 [`STORYBOARD_ALIGNMENT.md`](STORYBOARD_ALIGNMENT.md)에 정리했습니다.

### 추가

- Seed import/export 시 패스프레이즈 입력 다이얼로그를 추가했습니다(import 1회, export 입력+확인 2회 및 손실 경고). 빈 패스프레이즈는 허용하지 않으며 저장하지 않습니다.
- 가져오기 시 패스프레이즈 입력 전 `inspectHeader`로 레거시 평문 `.seed`를 거부합니다.
- seedcoat 오류 코드 전체에 대한 한국어 메시지 매핑(`src/constants/projectStorageMessages.ts`)을 추가했습니다.
- `.seed` 보내기/가져오기 시 KDF 대기용 진행 알림(`withProgress`)을 추가했습니다.
- `.seed` 보내기 전 `scenePrefixDigits`·씬 stem seedcoat 규칙 사전 검사(`src/files/seedExportPreflight.ts`)를 추가했습니다.
- [`EXTENSION_QA.md`](EXTENSION_QA.md)에 `.seed` 수동 QA 절, [`RELEASE.md`](RELEASE.md)에 `compile`·VSIX `.seed` smoke 절차를 추가했습니다.

### 참고

- 캐릭터 `arc` / `recentDialogues` / `profile` / `attributes`는 `.seed` 왕복에서 보존되지 않습니다(seedcoat가 encode 시 폐기). `trackDraft`는 seed `project` envelope에 없어 동기화 시 덮어쓰면 소실될 수 있습니다. 기존 워크스페이스의 구 형식 background 카드는 자동 마이그레이션되지 않습니다.

## [0.1.3] - 2026-05-21

### 추가

- Seed `.seed` 파일 포맷 v2 envelope의 공식 사양 문서(`SEED-FORMAT.md`)를 추가했습니다.
- Seed 파일에서 Storyboard 프로젝트를 생성하고 기존 프로젝트와 동기화하는 command를 추가했습니다.
- Storyboard 프로젝트를 Seed 파일로 내보내는 export command와 관련 문서를 추가했습니다.

## [0.1.2] - 2026-05-11

### 문서

- `README.md`와 `doc/`에서 Visual Studio Marketplace 발행 안내를 제거하고, GitHub Releases 전용 VSIX 배포 방침에 맞췄습니다.
- `README.md`와 `doc/plan.md`에 한국어 변경 내역 링크와 릴리스 노트 동기화 안내를 추가했습니다.

## [0.1.1] - 2026-05-09

### 추가

- 점진적 UI 업데이트를 위한 AI 스트리밍 RPC(`ai.generateStream`)와 청크 이벤트(`ai.generateStream.chunk`)를 추가했습니다. 네이티브 스트리밍을 지원하지 않는 provider는 registry를 통해 단일 응답 생성으로 fallback합니다.
- scene draft 생성 중 긴 `previousContext`를 선택적으로 줄이기 위한 `storyboard.ai.contextCondenseEnabled` 설정을 추가했습니다.

### 변경

- Prompt variant 선택이 provider 전용 routing 대신 task, model, token budget(`xs` / `generic` / `rich`)을 함께 고려하도록 변경했습니다.
- `rich` variant가 선택되면 persona dialogue와 genre formatting 등 장문 prompt가 더 풍부한 지시문을 사용하도록 변경했습니다.

## [0.1.0] - 2026-05-09

### 변경

- 문서: `doc/plan.md`, `doc/concept.md`, `doc/decisions/01-project-initialization.md`, `doc/testing/extension-qa.md`, `README.md`를 release policy에 맞게 정렬했습니다 — `.picktion` import 없음, extension UI i18n은 **`ko` 기본**, **`en` 선택**, 첫 Marketplace 목표는 **0.1.0**(Phase 7–8).
- 문서: `doc/plan.md`에서 Phase 7이 publish 전 LICENSE, privacy, Marketplace metadata를 담당하도록 하고, Phase 8을 `vsce publish` 직전 최종 QA gate로 정리했습니다.

### 추가

- tag 기반 VSIX packaging과 checksum asset을 생성하는 GitHub Release workflow를 추가했습니다.
- `@vscode/vsce` 기반 로컬 `npm run package:vsix` script를 추가했습니다.
- 기본 provider, provider별 curated model, task별 provider override, Ollama base URL, API key(SecretStorage), connection check를 관리하는 settings webview panel(`Storyboard: Open Settings`)을 추가했습니다. settings RPC와 `settings.changed` sync도 포함됩니다.
- cache metadata와 새 draft 생성 시 선택적 trait update를 포함하는 scene-to-draft generation pipeline을 추가했습니다(Phase 4).
- tree, status badge, in-view action을 제공하는 Scenes sidebar webview를 추가했습니다(Phase 5).
- Character relation graph panel(d3-force)과 `Storyboard: Open Character Relation Graph` command를 추가했습니다(Phase 5).
- draft가 있는 경우 `scene/*.txt`에서 generate/regenerate draft와 apply format을 실행할 수 있는 CodeLens를 추가했습니다(Phase 5).
- `draft/*.md`에서 re-generate, grammar check, expand를 실행할 수 있는 CodeLens를 추가했습니다. grammar/expand는 Phase 6 placeholder notice를 표시합니다(Phase 5).
- 0.0.1 dogfooding을 위한 manual MVP QA guide를 `doc/testing/extension-qa.md`에 추가했습니다(Phase 5).
- Storyboard VSCode extension의 repository baseline을 수립했습니다.
- 초기 TypeScript 및 esbuild extension-host scaffold를 추가했습니다.
- `storyboard.helloWorld` sanity-check command를 추가했습니다.
- F5 extension debugging을 위한 VSCode launch/tasks 설정을 추가했습니다.
- ESLint 및 Prettier baseline 설정을 추가했습니다.
- Storyboard workspace 구조를 생성하는 `storyboard.init` command를 추가했습니다.
- `.storyboard/project.json`의 project metadata validation을 추가했습니다.
- extension-host workspace, path convention, logger module을 추가했습니다.
- Storyboard Activity Bar container와 sidebar placeholder view를 추가했습니다.
- 최소 Vite 및 React webview UI build를 추가했습니다.
- `.card` YAML schema, round-trip test, Storyboard card custom editor를 추가했습니다.
- file watching과 card opening을 지원하는 Characters 및 Backgrounds sidebar view를 추가했습니다.
- sidebar에서 character/background card를 생성하는 command를 추가했습니다.
- `mock`을 기본 provider로 사용하는 Storyboard AI provider configuration key를 추가했습니다.
- SecretStorage 기반 API key 관리와 `Storyboard: Set API Key...` command를 추가했습니다.
- Phase 3 AI integration을 위한 testable `SecretStore` 및 `ConfigBridge` adapter를 추가했습니다.
- mock 및 OpenAI provider를 지원하는 Phase 3 AI provider registry를 추가했습니다.
- 초기 `ai.providers.list`, `ai.providers.checkConnection`, `ai.generate` RPC contract를 추가했습니다.
- Phase 3 AI registry에 Claude, Google Gemini, Ollama provider support를 추가했습니다.
- response parsing, JSON repair, trait processing을 위한 deterministic AI utility module을 추가했습니다.
- 최소 Storyboard AI service facade와 Phase 3 orchestration용 core prompt module을 추가했습니다.

### 문서

- README를 Phase 5 scope 및 0.0.1 dogfooding에 맞게 정렬했습니다(Marketplace publish는 아직 없음).
- `doc/plan.md` MVP gate를 local `vsce package`, QA, dogfooding 중심으로 재정의했습니다.

### 변경

- Project license를 MIT에서 Apache License 2.0으로 변경했습니다(`LICENSE`, `package.json`).
- Phase 0을 Picktion compatibility fixture 대신 repository readiness에 집중하도록 축소했습니다.
- `npm run build`가 extension host와 webview UI를 모두 bundle하도록 packaging script를 업데이트했습니다.
