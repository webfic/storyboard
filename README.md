...
같은 엔진을 두 가지 방식으로 씁니다 — VS Code 확장과 터미널 CLI.
...
작품 폴더는 git 저장소로 만들어지므로 되돌리기와 협업은 평소 쓰던 git 그대로입니다 — 다만 커밋은 도구가 아니라 사용자가 합니다.
...
`~/.local/share/`에 풀고 `~/.local/bin/storyboard`를 링크하며, 체크섬을 검증하고
...
```bash
storyboard init --title "밤의 항해"   # 빈 디렉터리가 작품(이자 git 저장소)이 됩니다
storyboard setup                      # AI 프로바이더와 키 — 두 앱이 함께 씁니다
